...
export default function VentasPage() {
  ...
  const [filtroTarjeta, setFiltroTarjeta] = useState<
    "Todas" | "Pendientes" | "Novedades" | "Aprobadas"
  >("Todas");
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        tarjetaURL === "Todas" ||
        tarjetaURL === "Pendientes" ||
        tarjetaURL === "Novedades" ||
        tarjetaURL === "Aprobadas"
      ) {
  ...
  const [factModal, setFactModal] = useState<{ open: boolean; venta: any }>({
    open: false,
    venta: null,
  });
  ...
  const handleSaveBaseManual = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formBase.identificacion || !formBase.nombres)
      return showToast("alerta", "La cédula y los nombres son obligatorios.");
  ...
  const [masivoModal, setMasivoModal] = useState({
    open: false,
    encontrados: [] as any[],
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filtros, filtroTarjeta]);
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!headers || headers.length === 0 || data.length === 0) {
          return showToast(
            "error",
            "El archivo de Excel está vacío o es inválido.",
          );
        }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      data.forEach((row: any) => {
        ...
        if (ventaDB) {
          const verificacion = String(
            row.Verificacion || row.verificacion || "",
          ).trim();
          const observaciones = row.Observaciones || row.observaciones || "";
          const estadoTicket =
            verificacion === contratoExcel
              ? "Validado ✅"
              : "Rechazado ❌ - Número no coincide";
          encontrados.push({
            id: ventaDB.id,
  ...
  const confirmarValidacionMasiva = async () => {
    ...
    try {
      ...
      showToast("error", "Error al procesar el Excel.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleOpenEdit = (v: any) => {
    setFormVenta({
      ...
      observacionesFact:
        v.observacionesFact === "Sin observaciones"
          ? ""
          : v.observacionesFact || "",
      verificacionFact:
        v.verificacionFact === "Sin validar" ? "" : v.verificacionFact || "",
    });
    setOpenCreate(true);
  };
  ...
  const handleCrearVenta = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formVenta.institucionId)
      return showToast("error", "Seleccione la escuela.");
    if (!formVenta.numContrato)
      return showToast("error", "Ingrese el N° de contrato.");
    ...
    try {
      ...
      showToast("error", "Hubo un problema al guardar.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleGuardarFacturacion = async () => {
    if (
      factData.verificacionFact &&
      factData.verificacionFact !== factModal.venta.numContrato
    ) {
      showToast(
        "alerta",
        `El número de verificación (${factData.verificacionFact}) no coincide con el contrato N° ${factModal.venta.numContrato}.`,
      );
      return;
    }
    try {
      await fetch("/api/ventas", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: factModal.venta.id,
          observacionesFact: factData.observacionesFact,
          verificacionFact: factData.verificacionFact,
        }),
      });
      ...
      showToast("error", "Hubo un error al guardar la auditoría.");
    }
  };
  ...
  const handleCrearTicket = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!ticketModal.tipo)
      return showToast("alerta", "Selecciona el Área Responsable");
    if (!ticketModal.mensajeInicial)
      return showToast("alerta", "Escribe las instrucciones de la novedad");
    setSaving(true);
    try {
  ...
  const puedeValidar =
    userPermisos.includes("ventas:validar") ||
    userRol === "super_admin" ||
    userRol === "administrador";
  const esAdmin = userRol === "super_admin" || userRol === "administrador";
  const puedeVerBase =
    userRol === "super_admin" || userPermisos.includes("base_consultas:ver");

  const vendedoresUnicos = Array.from(
    new Set(ventas.map((v) => v.vendedorNombre)),
  ).filter(Boolean);
  const escuelasDelCanton = formVenta.cantonId
    ? institucionesDisponibles.filter(
        (i) => i.cantonId === parseInt(formVenta.cantonId),
      )
    : [];

  const kpiPendientes = ventas.filter(
    (v) =>
      (!v.verificacionFact || v.verificacionFact === "Sin validar") &&
      (!v.observacionesFact ||
        v.observacionesFact === "Sin observaciones" ||
        v.observacionesFact.trim() === ""),
  ).length;
  const kpiNovedades = ventas.filter(
    (v) =>
      v.observacionesFact &&
      v.observacionesFact !== "Sin observaciones" &&
      v.observacionesFact.trim() !== "",
  ).length;
  const kpiAprobadas = ventas.filter(
    (v) => v.verificacionFact && v.verificacionFact !== "Sin validar",
  ).length;

  let ventasFiltradas = ventas.filter((v) => {
    if (filtroTarjeta === "Pendientes")
      return (
        (!v.verificacionFact || v.verificacionFact === "Sin validar") &&
        (!v.observacionesFact ||
          v.observacionesFact === "Sin observaciones" ||
    ...
    if (filtroTarjeta === "Novedades")
      return (
        v.observacionesFact &&
        v.observacionesFact !== "Sin observaciones" &&
        v.observacionesFact.trim() !== ""
      );
    if (filtroTarjeta === "Aprobadas")
      return v.verificacionFact && v.verificacionFact !== "Sin validar";
    return true;
  });

  if (searchTerm) {
    ventasFiltradas = ventasFiltradas.filter((v) =>
      String(v.numContrato).includes(searchTerm),
    );
  }
  if (filtros.vendedorNombre)
    ventasFiltradas = ventasFiltradas.filter(
      (v) => v.vendedorNombre === filtros.vendedorNombre,
    );
  if (filtros.estadoContratoId)
    ventasFiltradas = ventasFiltradas.filter(
      (v) => String(v.estadoContratoId) === filtros.estadoContratoId,
    );
  if (filtros.estadoClienteId)
    ventasFiltradas = ventasFiltradas.filter(
      (v) => String(v.estadoClienteId) === filtros.estadoClienteId,
    );
  if (filtros.mesCobro)
    ventasFiltradas = ventasFiltradas.filter(
      (v) => v.mesCobro === filtros.mesCobro,
    );
  const totalPages = Math.max(
    1,
    Math.ceil(ventasFiltradas.length / itemsPerPage),
  );
  ...
  const exportarExcel = () => {
    const dataToExport = ventasFiltradas.map((v) => ({
      ...
      "Observaciones / Novedades":
        v.observacionesFact !== "Sin observaciones" ? v.observacionesFact : "",
      "Entregó Cédula": v.tieneCedula ? "SÍ" : "NO",
      Cédula: v.numeroCedula,
    }));
  ...
  return (
    <>
          ...
          <>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                  <DollarSign className="text-emerald-600" /> Facturación y
                  Tickets de Venta
                </h1>
                <p className="text-xs md:text-sm text-gray-500 mt-0.5">
                  Control de contratos, validación y reportes gerenciales
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all ${filtroTarjeta === "Todas" ? "bg-blue-600 text-white shadow-md" : "bg-white hover:border-blue-300"}`}
              >
                <p
                  className={`text-[10px] font-bold uppercase ${filtroTarjeta === "Todas" ? "text-blue-100" : "text-gray-500"}`}
                >
                  📘 Total Ventas
                </p>
              ...
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all ${filtroTarjeta === "Pendientes" ? "bg-gray-600 text-white shadow-md" : "bg-white hover:border-gray-400"}`}
              >
                <p
                  className={`text-[10px] font-bold uppercase ${filtroTarjeta === "Pendientes" ? "text-gray-200" : "text-gray-500"}`}
                >
              ...
              <div
                onClick={() => setFiltroTarjeta("Novedades")}
                className={`p-4 rounded-xl border cursor-pointer transition-all ${filtroTarjeta === "Novedades" ? "bg-amber-500 text-white shadow-md" : "bg-white hover:border-amber-400"}`}
              >
                <p
                  className={`text-[10px] font-bold uppercase ${filtroTarjeta === "Novedades" ? "text-amber-100" : "text-amber-600"}`}
                >
                  ⚠️ Con Novedades
                </p>
              ...
              <div
                onClick={() => setFiltroTarjeta("Aprobadas")}
                className={`p-4 rounded-xl border cursor-pointer transition-all ${filtroTarjeta === "Aprobadas" ? "bg-emerald-600 text-white shadow-md" : "bg-white hover:border-emerald-300"}`}
              >
                <p
                  className={`text-[10px] font-bold uppercase ${filtroTarjeta === "Aprobadas" ? "text-emerald-100" : "text-emerald-600"}`}
                >
                  ✅ Aprobadas / Validadas
                </p>
            ...
            {/* --- FILTROS --- */}
            <div
              className={`bg-white p-4 rounded-xl border border-gray-200 shadow-sm grid grid-cols-1 gap-3 ${esAdmin ? "md:grid-cols-5" : "md:grid-cols-4"}`}
            >
              ...
              {esAdmin && (
                <div>
                  ...
                  <select
                    className="w-full h-9 border rounded-md px-2 text-xs mt-1 bg-white outline-none"
                    value={filtros.vendedorNombre}
                    onChange={(e) =>
                      setFiltros({ ...filtros, vendedorNombre: e.target.value })
                    }
                  >
              ...
              <div>
                ...
                <select
                  className="w-full h-9 border rounded-md px-2 text-xs mt-1 bg-white outline-none"
                  value={filtros.estadoContratoId}
                  onChange={(e) =>
                    setFiltros({ ...filtros, estadoContratoId: e.target.value })
                  }
                >
                  <option value="">Todos</option>
                  {catalogos?.estadosContrato?.map((e: any) => (
              ...
              <div>
                ...
                <select
                  className="w-full h-9 border rounded-md px-2 text-xs mt-1 bg-white outline-none"
                  value={filtros.estadoClienteId}
                  onChange={(e) =>
                    setFiltros({ ...filtros, estadoClienteId: e.target.value })
                  }
                >
                  <option value="">Todos</option>
                  {catalogos?.estadosCliente?.map((e: any) => (
                    <option key={e.id} value={e.id}>
                      {e.nombre}
                    </option>
                  ))}
              ...
              <div>
                ...
                <select
                  className="w-full h-9 border rounded-md px-2 text-xs mt-1 bg-white outline-none"
                  value={filtros.mesCobro}
                  onChange={(e) =>
                    setFiltros({ ...filtros, mesCobro: e.target.value })
                  }
                >
                  <option value="">Todos</option>
                  {MESES_LISTA.map((m) => (
              ...
              <Table className="w-full table-fixed min-w-1000px">
                <TableHeader className="bg-gray-50/80">
                  <TableRow>
                    ...
                    <TableHead className="font-semibold text-gray-700 text-center w-32">
                      Valor / Cuota
                    </TableHead>
                    <TableHead className="font-semibold text-gray-700 text-center w-28">
                      Cobro / Mes
                    </TableHead>
                    <TableHead className="font-semibold text-gray-700 text-center w-32">
                      Estados
                    ...
                    ventasPaginadas.map((v) => (
                      <TableRow key={v.id} className="hover:bg-gray-50">
                        <TableCell className="w-44">
                          ...
                          <div
                            className="text-xs text-primary font-semibold truncate"
                            title={v.institucionNombre}
                          >
                            {v.institucionNombre}
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-center w-32">
                          <div className="text-sm font-extrabold text-emerald-700">
                            ${v.valorContrato.toFixed(2)}
                          </div>
                          <div className="text-[12px] text-gray-500">
                            {v.meses} m x ${v.cuotaMensual.toFixed(2)}
                          </div>
                        </TableCell>
                        <TableCell className="text-center w-28">
                          <Badge
                            variant="outline"
                            className="bg-blue-50 text-blue-700 border-blue-200 text-[10px] truncate max-w-full block"
                          >
                            {v.tipoCobroNombre}
                          </Badge>
                        ...
                        <TableCell className="text-center w-32">
                          <span
                            className="block text-[10px] px-1.5 py-0.5 rounded font-bold bg-purple-50 text-purple-700 border border-purple-200 mb-1 truncate"
                            title={v.estadoClienteNombre}
                          >
                            Cli: {v.estadoClienteNombre}
                          </span>
                          <span
                            className="block text-[10px] px-1.5 py-0.5 rounded font-bold bg-amber-50 text-amber-700 border border-amber-200 truncate"
                            title={v.estadoContratoNombre}
                          >
                            Con: {v.estadoContratoNombre}
                          </span>
                        </TableCell>
                        <TableCell className="text-center w-28">
                          <span
                            className={`text-xs px-2 py-1 rounded-md font-bold block truncate ${v.verificacionFact !== "Sin validar" ? "bg-emerald-100 text-emerald-700 border border-emerald-300" : "bg-gray-100 text-gray-500"}`}
                          >
                            {v.verificacionFact !== "Sin validar"
                              ? v.verificacionFact
                              : "Pendiente"}
                          </span>
                        </TableCell>
                        <TableCell className="w-48 max-w-192px align-middle">
                          <div
                            className={`text-[11px] font-medium italic overflow-wrap: break-word line-clamp-2 max-w-192px overflow-hidden ${v.observacionesFact !== "Sin observaciones" && v.observacionesFact.trim() !== "" ? "text-amber-700 bg-amber-50 p-1.5 rounded border border-amber-200" : "text-gray-400"}`}
                        ...
                        <TableCell className="text-center w-24 align-middle">
                          ...
                          {v.tieneCedula && v.numeroCedula && (
                            <span className="block text-[13px] font-mono font-bold text-gray-600 mt-1">
                              {v.numeroCedula}
                            </span>
                          )}
                        </TableCell>
                        <TableCell className="text-center w-32">
                          <div className="flex justify-center gap-1">
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-8 w-8 text-amber-600 border-amber-200 hover:bg-amber-50 p-0"
                              onClick={() => handleOpenTicket(v)}
                              title="Crear Ticket de Novedad"
                            >
                              <Ticket size={14} />
                            </Button>
                            <Button
                            ...
                            {puedeValidar && (
                              <Button
                                variant="outline"
                                size="sm"
                                className="h-8 text-xs text-primary border-primary/30 px-2"
                                onClick={() => {
                                  setFactData({
                                    observacionesFact:
                                      v.observacionesFact ===
                                  ...
                                  setFactModal({ open: true, venta: v });
                                }}
                              >
              ...
              {totalPages > 1 && (
                <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 bg-gray-50/50 rounded-b-xl">
                  <span className="text-xs text-gray-500 font-medium">
                    ...
                    de {ventasFiltradas.length} contratos
                  </span>
        ...
        {activeTab === "base" && puedeVerBase && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                  <Database className="text-emerald-600" /> Base de Consultas de
                  Clientes
                </h1>
                <p className="text-sm text-gray-500 mt-1">
                  Busca capacidad de endeudamiento y estado laboral.
                </p>
              </div>
              ...
              {esAdmin && (
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    className="hidden"
                    ref={fileBaseRef}
                    onChange={handleUploadBaseExcel}
                  />
                  <Button
                    onClick={() => fileBaseRef.current?.click()}
                  ...
                  >
                    ...
                    {uploadingBase ? "Procesando..." : "Subir Base (Excel)"}
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row items-center gap-4">
              <div className="relative w-full md:w-1/2">
                ...
                <div className="relative mt-1">
                  ...
                  />
                  <Input
                    className="pl-9 h-10 bg-gray-50 focus:bg-white border-gray-300 focus-visible:ring-emerald-500 font-semibold"
                    onChange={(e) => {
                      setSearchBase(e.target.value);
                      setCurrentPageBase(1);
                    }}
                    placeholder="Escribe el nombre o número de cédula del cliente..."
                    value={searchBase}
                  />
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  {baseConsultas
                    .filter(
                      (c) =>
                        c.nombres
                          .toLowerCase()
                          .includes(searchBase.toLowerCase()) ||
                        c.identificacion.includes(searchBase),
                    )
                    .slice(
                      (currentPageBase - 1) * itemsPerPage,
                      currentPageBase * itemsPerPage,
                    )
                    .map((cliente) => (
                      <TableRow className="hover:bg-gray-50" key={cliente.id}>
                        ...
                        <TableCell className="text-center">
                          <Badge
                            className="bg-gray-100 text-gray-700 border-gray-200 uppercase text-[10px]"
                            variant="outline"
                          >
                        ...
                        <TableCell className="text-center">
                          <Button
                            ...
                            onClick={() => handleOpenEditBase(cliente)}
                            title="Editar Cliente / Vincular Escuela"
                          >
                            <Pencil size={14} />
                          </Button>
                        </TableCell>
              ...
              {Math.ceil(
                ...
                <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 bg-gray-50/50 rounded-b-xl">
                  <span className="text-xs text-gray-500 font-medium">
                    Mostrando {(currentPageBase - 1) * itemsPerPage + 1} a{" "}
                    {Math.min(
                      currentPageBase * itemsPerPage,
                      baseConsultas.filter(
                        (c) =>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setCurrentPageBase((p) => Math.max(1, p - 1))
                      }
        ...
        <DialogContent className="sm:max-w-xl bg-white p-6 rounded-xl border-t-4 border-t-emerald-500">
          ...
          <div className="mt-4 space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-200">
            ...
            <div className="flex items-center justify-between">
              ...
              <select
                className="w-1/2 h-9 border border-gray-300 rounded-md px-2 text-xs bg-white focus:ring-emerald-500 outline-none"
                value={baseMapping.identificacion}
                onChange={(e) =>
                  setBaseMapping({
                    ...baseMapping,
            ...
            <div className="flex items-center justify-between">
              ...
              <select
                className="w-1/2 h-9 border border-gray-300 rounded-md px-2 text-xs bg-white focus:ring-emerald-500 outline-none"
                value={baseMapping.nombres}
                onChange={(e) =>
                  setBaseMapping({ ...baseMapping, nombres: e.target.value })
                }
              >
                <option value="">Seleccione...</option>
                {baseExcelColumns.map((col) => (
            ...
            <div className="flex items-center justify-between">
              ...
              <select
                className="w-1/2 h-9 border border-gray-300 rounded-md px-2 text-xs bg-white focus:ring-emerald-500 outline-none"
                value={baseMapping.liquidoPagar}
                onChange={(e) =>
                  setBaseMapping({
                    ...baseMapping,
                    liquidoPagar: e.target.value,
                  })
                }
              >
                <option value="">-- Ignorar si no existe --</option>
                {baseExcelColumns.map((col) => (
                  <option key={col} value={col}>
                    {col}
                  </option>
            ...
            <div className="flex items-center justify-between">
              ...
              <select
                className="w-1/2 h-9 border border-gray-300 rounded-md px-2 text-xs bg-white focus:ring-emerald-500 outline-none"
                value={baseMapping.modalidad}
                onChange={(e) =>
                  setBaseMapping({ ...baseMapping, modalidad: e.target.value })
                }
              >
                <option value="">-- Ignorar si no existe --</option>
                {baseExcelColumns.map((col) => (
            ...
            <div className="flex items-center justify-between">
              ...
              <select
                className="w-1/2 h-9 border border-gray-300 rounded-md px-2 text-xs bg-white focus:ring-emerald-500 outline-none"
                value={baseMapping.escuela}
                onChange={(e) =>
                  setBaseMapping({ ...baseMapping, escuela: e.target.value })
                }
              >
                <option value="">-- Ignorar si no existe --</option>
                {baseExcelColumns.map((col) => (
          ...
          <DialogFooter className="mt-6 flex justify-end gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setBaseImportModal(false)}
        ...
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-xl border-t-4 border-t-red-500">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-red-900">
              {formBase.id ? "Editar Cliente" : "Nuevo Cliente (Manual)"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSaveBaseManual} className="space-y-4 mt-2">
            <div className="grid grid-cols-1 gap-3">
              <div>
                ...
                <Input
                  required
                  value={formBase.identificacion}
                  onChange={(e) =>
                    setFormBase({ ...formBase, identificacion: e.target.value })
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  className="h-10 text-xs mt-1 uppercase"
              ...
              <div>
                ...
                <Input
                  type="number"
                  step="0.01"
                  value={formBase.liquidoPagar}
                  onChange={(e) =>
                    setFormBase({ ...formBase, liquidoPagar: e.target.value })
                  }
                  className="h-10 text-xs mt-1 font-bold text-emerald-700"
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <select
                  className="w-full h-10 border border-gray-300 rounded-md px-3 text-xs bg-white mt-1 outline-none focus:border-emerald-500"
                  value={formBase.modalidadId}
                  onChange={(e) =>
                    setFormBase({ ...formBase, modalidadId: e.target.value })
                  }
                >
                  <option value="">-- No Especificado --</option>
                  {catalogos?.modalidadesLaborales?.map((m: any) => (
            ...
            <DialogFooter className="mt-5 flex gap-2 justify-end">
              <Button
                type="button"
                variant="outline"
                onClick={() => setModalBaseManual(false)}
              >
                Cancelar
              </Button>
              <Button
          ...
          <form onSubmit={handleCrearVenta} className="space-y-4 mt-2">
            <div className="grid grid-cols-2 gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
              <div>
                ...
                <select
                  required
                  disabled={!!formVenta.id}
                  className="w-full h-10 border rounded-md px-2 text-xs bg-white mt-1 disabled:bg-gray-100 disabled:text-gray-500"
                  value={formVenta.cantonId}
                  onChange={(e) =>
                    setFormVenta({
                      ...formVenta,
                      cantonId: e.target.value,
                      institucionId: "",
                    })
                  }
                >
              ...
              <div>
                <Label className="text-xs font-bold text-gray-700">
                  2. Escuela / Institución *
                </Label>
                <select
                  required
                  disabled={!!formVenta.id || !formVenta.cantonId}
                  className="w-full h-10 border rounded-md px-2 text-xs bg-white mt-1 disabled:bg-gray-100 disabled:text-gray-500"
                ...
                >
                  ...
                  {escuelasDelCanton.map((i: any) => (
                    <option key={i.id} value={i.id}>
                      {i.nombre}
                    </option>
                  ))}
                  {!!formVenta.id &&
                    !escuelasDelCanton.some(
                      (i) => i.id == formVenta.institucionId,
                    ) && (
                      <option value={formVenta.institucionId}>
                        {formVenta.institucionNombre || "Escuela Actual"}
                      </option>
                    )}
                </select>
              </div>
            ...
            <div
              className={`p-4 rounded-xl border ${formVenta.observacionesFact ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-gray-200"}`}
            >
              <span
                className={`text-[11px] font-bold uppercase 'block' mb-2 flex items-center gap-1 ${formVenta.observacionesFact ? "text-amber-800" : "text-gray-600"}`}
              >
                <ShieldCheck size={14} />{" "}
                {puedeValidar
                  ? "Sección de Auditoría (Tú puedes editar)"
                  : "Notas de Auditoría (Solo Lectura)"}
              </span>
                ...
                <div className="md:col-span-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormVenta({
                        ...formVenta,
                        verificacionFact: e.target.value.replace(/\D/g, ""),
                      })
                    }
                    placeholder="Ej: 100234"
                  />
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setFormVenta({
                        ...formVenta,
                        observacionesFact: e.target.value,
                      })
                    }
                    placeholder={
                      puedeValidar
                        ? "Escribe aquí si hay errores..."
      ...
      <Dialog
        open={masivoModal.open}
        onOpenChange={(val) => setMasivoModal({ ...masivoModal, open: val })}
      >
        <DialogContent className="sm:max-w-2xl bg-white p-6 rounded-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
              <h3 className="font-bold text-emerald-800 flex items-center gap-2">
                <CheckCircle2 size={18} /> {masivoModal.encontrados.length}{" "}
              ...
              <div className="mt-2 max-h-32 overflow-y-auto text-[10px] font-mono bg-white p-2 rounded border border-emerald-200">
                {masivoModal.encontrados.map((n, i) => (
                  <div key={i}>Contrato N° {n.contrato}</div>
                ))}
              </div>
            </div>
            {masivoModal.noEncontrados.length > 0 && (
              <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                <h3 className="font-bold text-red-800 flex items-center gap-2">
                  <AlertCircle size={18} /> {masivoModal.noEncontrados.length}{" "}
                  Contratos NO Encontrados
                </h3>
                <p className="text-xs text-red-600 mt-1 mb-2">
                  No existen en la base de datos. Haz clic en "Registrar" para
                  ...
                  {masivoModal.noEncontrados.map((n, i) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleOpenCreateFromExcel(n.contratoExcel)
                        }
                      >
          ...
          <DialogFooter className="mt-4 flex gap-2 justify-end">
            ...
            <Button
              ...
              disabled={saving || masivoModal.encontrados.length === 0}
              onClick={confirmarValidacionMasiva}
            >
      ...
      <Dialog
        open={factModal.open}
        onOpenChange={(val) => setFactModal({ ...factModal, open: val })}
      >
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          ...
          <div className="space-y-3 mt-3">
            <div>
              <Label className="text-xs font-semibold text-red-600">
                Verificación (Si es incorrecto, el sistema te bloqueará)
              </Label>
              <Input
                type="text"
                pattern="[0-9]+"
                placeholder="Ej: 100234"
                value={factData.verificacionFact}
                onChange={(e) =>
                  setFactData({
                    ...factData,
                    verificacionFact: e.target.value.replace(/\D/g, ""),
            ...
            <div>
              ...
              <textarea
                className="w-full border rounded-md p-2 text-xs bg-white h-24 outline-none focus:ring-2 focus:ring-amber-400"
                value={factData.observacionesFact}
                onChange={(e) =>
                  setFactData({
                    ...factData,
                    observacionesFact: e.target.value,
          ...
          <DialogFooter className="mt-4">
            <Button
              variant="outline"
              onClick={() => setFactModal({ open: false, venta: null })}
            >
              Cancelar
            </Button>
            <Button
      ...
      <Dialog
        ...
        onOpenChange={(val) => setTicketModal({ ...ticketModal, open: val })}
      >
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          <DialogHeader>
          ...
          <form onSubmit={handleCrearTicket} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  className="w-full h-10 border rounded-md px-2 text-xs bg-white mt-1 outline-none"
                  value={ticketModal.tipo}
                  onChange={(e) =>
                    setTicketModal({ ...ticketModal, tipo: e.target.value })
                  }
                >
                  {departamentos.map((d) => (
            ...
            <div>
              ...
              <Input
                required
                value={ticketModal.asunto}
                onChange={(e) =>
                  setTicketModal({ ...ticketModal, asunto: e.target.value })
                }
                className="mt-1 h-10 font-bold bg-gray-50"
              />
            </div>
            <div>
              <Label className="text-xs font-bold text-gray-700">
                Instrucciones / Novedad *
              </Label>
              <textarea
                required
                className="w-full h-24 border rounded-md p-2 text-xs bg-white mt-1 outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 resize-none"
                placeholder="Ej: Falta copia de cédula..."
                value={ticketModal.mensajeInicial}
                onChange={(e) =>
                  setTicketModal({
                    ...ticketModal,

...
export default function VisitasPage() {
  ...
  const cargarDatos = async (isSilent = false) => {
    ...
    try {
      ...
      if (filtros.institucionId)
        params.append("institucionId", filtros.institucionId);
      ...
      if (instituciones.length === 0)
        fetchPromises.push(fetch("/api/instituciones?limit=500"));
      if (vendedores.length === 0)
        fetchPromises.push(fetch("/api/usuarios/vendedores"));

      const responses = await Promise.all(fetchPromises);
      const resVisitas = await responses[0].json();

      const filtrarVisitasReales = (lista: any[]) =>
        lista.filter((v) => {
      ...
      if (Array.isArray(resVisitas)) {
        ...
        const paginatedData = visitasReales.slice(
          (currentPage - 1) * itemsPerPage,
          currentPage * itemsPerPage,
        );
        setVisitas(paginatedData);
        setTotalMonto(
          visitasReales.reduce(
            (sum: number, v: any) => sum + (v.totalVendido || 0),
            0,
          ),
        );
      } else {
  ...
  const handleToggleCandado = async (
    visitaId: string,
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      let visitasFull = Array.isArray(dataFull)
        ? dataFull
        : dataFull.data || [];
      ...
      const dataParaExcel = visitasFull.map((v: any) => ({
        ...
        "Ubicación GPS": v.latitud
          ? `https://maps.google.com/?q=${v.latitud},${v.longitud}`
          : "Sin GPS",
      }));
  ...
  const cantonesDisponibles =
    catalogos?.provincias?.flatMap((p: any) => p.cantones) || [];
    ...
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-background">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-h1 flex items-center gap-2">
            <MapPin className="text-primary" /> Auditoría de Campo y
            Seguimientos
          </h1>
          <p className="text-secondary mt-0.5">
            Reporte consolidado que cruza las visitas en ruta con los contratos
            de ventas generados.
          </p>
        </div>
        <Button
      ...
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex flex-wrap items-end gap-3">
        <div>
          ...
          <Input
            type="date"
            className="h-9 text-xs w-36 mt-1 bg-white"
        ...
        <div>
          ...
          <select
            className="h-9 border rounded-md px-2 text-xs bg-white mt-1 w-32 outline-none"
        ...
        <div>
          ...
          <select
            className="h-9 border rounded-md px-2 text-xs bg-white mt-1 w-48 outline-none"
            value={filtros.institucionId}
            onChange={(e) =>
              setFiltros({ ...filtros, institucionId: e.target.value })
            }
          >
            ...
            {instituciones
              .filter(
                (i) =>
                  !filtros.cantonId ||
                  i.cantonId === parseInt(filtros.cantonId),
              )
              .map((i) => (
        ...
        {catalogos?.userRol !== "vendedor" && (
          <div>
            ...
            <select
              className="h-9 border rounded-md px-2 text-xs bg-white mt-1 w-40 outline-none"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex items-center justify-between transition-transform hover:-translate-y-1">
          <div>
            ...
            <h3 className="text-2xl font-extrabold text-emerald-600">
              {visitas.filter((v) => v.ventasRegistradas > 0).length}
            </h3>
          </div>
        ...
        <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex items-center justify-between sm:col-span-2 transition-transform hover:-translate-y-1">
          <div>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">
              MONTO TOTAL CERRADO EN ESTAS VISITAS
            </p>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-x-auto relative">
        {isRefreshing && (
          <div className="absolute top-0 left-0 w-full h-1 bg-primary/20 z-10">
        ...
        <Table className={isRefreshing ? "opacity-80" : ""}>
          <TableHeader className="bg-muted/50">
            <TableRow>
              ...
              <TableHead className="font-semibold text-center bg-emerald-50/50 w-180px">
                Venta Cerrada
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              visitas.map((v, i) => (
                <TableRow key={i} className="hover:bg-muted/30">
                  ...
                  {catalogos?.userRol !== "vendedor" && (
                    <TableCell className="text-center align-middle">
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          handleToggleCandado(v.id, v.edicionFechaHabilitada)
                        }
                        title={
                          v.edicionFechaHabilitada
                            ? "Bloquear edición"
                            : "Habilitar corrección de fecha al vendedor"
                        }
                  ...
                  <TableCell className="align-top pt-4">
                    ...
                    {v.latitud && (
                      <a
                        href={`http://maps.google.com/?q=${v.latitud},${v.longitud}`}
                        target="_blank"
                  ...
                  <TableCell className="max-w-xs align-top pt-4">
                    ...
                    <div className="text-[11px] text-muted-foreground bg-muted/30 p-2 rounded-lg border">
                      {v.resumen}
                    </div>
                  </TableCell>
                  <TableCell
                    className={`text-center align-top pt-4 ${v.ventasRegistradas > 0 ? "bg-emerald-50/30" : ""}`}
                  >
                    {v.ventasRegistradas > 0 ? (
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[9px] bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded font-bold my-1">
                          {v.ventasRegistradas} Contrato(s)
                        </span>
                        <span
                          className="text-[9px] text-muted-foreground"
                          title={v.detallesContratos}
                        >
                          Ref: {v.detallesContratos}
                        </span>
                      </div>
                    ) : (
                      <span className="text-xs text-muted-foreground italic mt-3 block">
                        No hubo venta
                      </span>
                    )}
                  </TableCell>

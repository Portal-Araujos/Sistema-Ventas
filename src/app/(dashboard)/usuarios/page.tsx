...
export default function UsuariosPage() {
    ...
    try {
      ...
      if (!data.error) {
        setConfigSeguridad({
          ...
          rolesBloqueados: data.config?.rolesBloqueados ? JSON.parse(data.config.rolesBloqueados) : []
        });
  ...
  const handleGuardarConfigSeguridad = async () => {
    ...
    finally { setSavingSeguridad(false); }
  };
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-gray-50/30">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Controla tu equipo y protege el sistema contra accesos no autorizados.</p>
        ...
        <div className="animate-in fade-in duration-300">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {loadingUsers ? <TableRow><TableCell colSpan={4} className="text-center py-8 font-bold text-gray-500 animate-pulse">Cargando...</TableCell></TableRow> : 
                  usuarios.map(u => (
                      ...
                      <TableCell className="text-center">
                        {u.rolNombre !== 'super admin' && u.rolNombre !== 'super_admin' ? (
                          <div className="flex justify-center gap-2">
                            ...
                            <Button 
                              variant="outline" size="sm" onClick={() => handleToggleBloqueo(u.id, u.nombre, u.bloqueado)} 
                            ...
                            >
                              {u.bloqueado ? <Unlock size={16} /> : <Lock size={16} />}
                            </Button>
                          </div>
                        ) : (
                          <div className="flex justify-center" title="Super Admin Protegido"><ShieldAlert size={20} className="text-gray-300" /></div>
                        )}
                      </TableCell>
                    </TableRow>
                  ))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-5">
              ...
              <div>
                <Label className="text-xs font-bold text-gray-700 uppercase mb-2 block">Roles Restringidos a esta IP</Label>
              ...
              <Button onClick={handleGuardarConfigSeguridad} disabled={savingSeguridad} className="w-full bg-gray-900 hover:bg-black text-white font-bold h-11">
                <Save size={16} className="mr-2"/> {savingSeguridad ? 'Guardando...' : 'Aplicar Seguridad'}
              </Button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-0">
              <Table>
                <TableHeader className="bg-slate-50">
                  <TableRow>
                    <TableHead className="font-bold text-xs text-gray-600">Fecha y Hora</TableHead>
                    <TableHead className="font-bold text-xs text-gray-600">IP del Atacante</TableHead>
                    <TableHead className="font-bold text-xs text-gray-600">Evento</TableHead>
                    <TableHead className="font-bold text-xs text-gray-600 text-center">Castigo</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      <TableRow key={log.id} className="hover:bg-red-50/30">
                        ...
                        <TableCell>
                          {log.evento === 'RATE_LIMIT_ACTIVADO' ? (
                            ...
                            <Badge className="bg-amber-100 text-amber-800 border-amber-200 text-[10px]">INTENTO FALLIDO</Badge>
                          )}
        ...
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleGuardarUsuario} className="space-y-4 mt-2">
            <div>
              ...
              <Input required placeholder="Ej. Juan Pérez" value={formUser.nombre} onChange={e => setFormUser({ ...formUser, nombre: e.target.value })} className="mt-1 h-10" />
            </div>
            <div>
              <Label className="text-xs font-bold text-gray-600 uppercase">Correo Electrónico *</Label>
              <Input required type="email" placeholder="juan@empresa.com" value={formUser.email} onChange={e => setFormUser({ ...formUser, email: e.target.value })} className="mt-1 h-10" />
            </div>
            <div>
            ...
            <DialogFooter className="pt-4 flex justify-end gap-2">
              ...
              <Button type="submit" disabled={savingUser} className="bg-primary text-white font-bold">{savingUser ? 'Guardando...' : 'Guardar'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>

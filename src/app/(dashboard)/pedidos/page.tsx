...
function PedidosPageContent() {
  ...
  const [tabActiva, setTabActiva] = useState<"Borrador" | "Operaciones">(
    "Borrador",
  );
  ...
  //  ESTADOS PARA TICKET CONTEXTUAL
  const [modalTicketOpen, setModalTicketOpen] = useState(false);
  const [prendaParaTicket, setPrendaParaTicket] = useState<any>(null);
  const [departamentosLista, setDepartamentosLista] = useState<any[]>([]);
  ...
  const cargarDatos = async (silencioso = false) => {
    ...
    try {
      ...
      if (modalEditOpen && grupoEdit) {
        const grupoActualizado = dataTabla.find(
          (g: any) => g.id === grupoEdit.id,
        );
        if (grupoActualizado) setGrupoEdit(grupoActualizado);
      }
      if (modalRecepcionOpen && grupoRecepcion) {
        const recepActualizada = dataTabla.find(
          (g: any) => g.id === grupoRecepcion.id,
        );
  ...
  useEffect(() => {
    cargarDatos();
  }, [tabActiva, fechaDesde, fechaHasta, selectedInstFilter]);

  // RESETEAR PÁGINA AL FILTRAR
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedInstFilter, tabActiva]);
  ...
  const handleOpenEdit = (grupo: any) => {
    setGrupoEdit(grupo);
    setGlobalFechaRequerida(
      grupo.fechaRequerida ? grupo.fechaRequerida.split("T")[0] : "",
    );
  ...
  const handleEliminarPedido = async (grupo: any) => {
    ...
    try {
      const res = await fetch(
        `/api/pedidos?institucionId=${grupo.institucionId}&vendedorId=${grupo.vendedorId}`,
        { method: "DELETE" },
      );
  ...
  const handleConfirmarRecepcion = async () => {
    ...
    try {
      ...
      showToast("error", "Error al confirmar recepción.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDesbloquearRecepcion = async (prendaId: string) => {
    if (
      !confirm(
        '¿Seguro que deseas quitarle el "Check" de recibido a esta prenda?',
      )
    )
      return;
    try {
  ...
  const imprimirActaRecepcionPDF = (grupo: any) => {
    ...
    if (prendasRecibidas.length === 0) {
      return showToast(
        "error",
        'No hay prendas con estado "Recibido" para generar el acta.',
      );
    }

    const printWindow = window.open("", "_blank");
    if (!printWindow)
      return showToast("error", "El navegador bloqueó la ventana.");

    let htmlFilas = "";
    prendasRecibidas.forEach((p) => {
      const fechaRec = new Date(p.fechaRecepcion).toLocaleString("es-EC", {
        timeZone: "America/Guayaquil",
      });
      htmlFilas += `
        <tr>
  ...
  const calcularEstadisticas = (item: any) => {
    ...
    const estadoMostrar =
      item.estado === "Borrador"
        ? "Borrador"
        : estadosUnicos.size > 1
          ? "Varios Estados"
          : Array.from(estadosUnicos)[0] || item.estado;
  ...
  const getEstadoColor = (estado: string) => {
    ...
    if (e.includes("borrador"))
      return "bg-amber-100 text-amber-800 border-amber-200";
    if (e.includes("despacho") || e.includes("entregado"))
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredGrupos.length / itemsPerPage),
  );
    ...
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-gray-50/30">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Órdenes independientes por cliente y escuela.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col lg:flex-row gap-3 items-center justify-between">
        <div className="relative flex-1 w-full">
          ...
          <Input
            ...
            placeholder="Buscar por Institución o Código PED-..."
            value={searchTerm}
        ...
        <div className="flex items-center gap-2 bg-blue-50/50 p-1.5 rounded-xl border border-blue-100 w-full lg:w-auto">
          ...
          <div className="flex items-center gap-2">
            <Input
              type="date"
              className="h-8 text-[11px] bg-white border-blue-200 w-125px"
              value={fechaDesde}
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 rounded-2xl border border-dashed border-gray-300 text-center text-gray-500">
          ...
          <p className="font-bold text-lg text-gray-700">
            No hay escuelas en esta sección.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse text-xs min-w-800px">
              <thead className="sticky top-0 z-20 bg-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1)]">
                <tr className="text-gray-600 font-black uppercase border-b border-gray-300">
                  ...
                  <th className="p-3.5 text-center bg-gray-100">
                    Estado y Resumen
                  </th>
                        ...
                        <div className="flex flex-col items-center justify-center relative group">
                          ...
                          {item.pedidosAsociados?.some((p: any) =>
                            ...
                            <div className="mt-1 flex items-center justify-center cursor-help">
                              ...
                              />
                              <span className="ml-1 text-[9px] font-black text-red-500 uppercase tracking-wide">
                                Aviso
                              </span>
                              ...
                              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-56 sm:w-72 bg-gray-900 text-white text-xs rounded-xl p-3 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none">
                                ...
                                <p className="font-bold text-red-400 mb-2 border-b border-gray-700 pb-1 flex items-center gap-1">
                                  <AlertCircle size={14} /> Motivo de
                                  Reprogramación:
                                </p>
                                <div className="space-y-2 max-h-32 overflow-y-auto text-left">
                                  {item.pedidosAsociados
                                    .filter((p: any) => p.observacion?.trim())
                                    .map((p: any, idx: number) => (
                                      <p key={idx} className="leading-tight">
                                        <span className="text-gray-400 font-bold">
                                          Contrato #{p.numContrato}:
                                        </span>{" "}
                                        {p.observacion}
                                      </p>
                                    ))}
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          {tabActiva === "Operaciones" && stats.desp > 0 && (
                            <Button
                              ...
                              variant="ghost"
                              title={
                                stats.desp > stats.rec
                                  ? "Confirmar Llegada"
                                  : "Ver Recepciones"
                              }
                              className={`h-8 w-8 ${stats.desp > stats.rec ? "text-amber-500 hover:bg-amber-50" : "text-emerald-600 hover:bg-emerald-50"}`}
                              onClick={() => handleOpenRecepcion(item)}
                            >
                          ...
                          {item.estado === "Borrador" && (
                            <Button
                              size="icon"
                              variant="ghost"
                              title="Enviar Todo a Operaciones"
                              className="h-8 w-8 text-emerald-600 hover:bg-emerald-50"
                              onClick={() => handleEnviarOperaciones(item)}
                            >
                              <Send size={15} />
                            </Button>
                          )}
                          ...
                          {item.estado === "Borrador" && (
                            <Button
                              ...
                              onClick={() => handleEliminarPedido(item.id)}
                            >
                              <Trash2 size={15} />
                            </Button>
                          )}
          ...
          {totalPages > 1 && (
            <div className="sticky bottom-0 z-20 p-4 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-3 bg-white/95 backdrop-blur-sm shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
              <span className="text-xs text-gray-500 font-medium">
                ...
                {Math.min(currentPage * itemsPerPage, filteredGrupos.length)} de{" "}
                {filteredGrupos.length} pedidos
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border text-xs">
              ...
              <div>
                ...
                <span className="font-extrabold text-blue-700">
                  {grupoDetalle?.fechaRequeridaTexto}
                </span>
              </div>
              <div>
                <span className="text-gray-400 block font-bold uppercase">
      ...
      <Dialog open={modalRecepcionOpen} onOpenChange={setModalRecepcionOpen}>
        <DialogContent className="sm:max-w-5xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[85vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900 border-b pb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ClipboardCheck className="text-amber-500" /> Confirmación de
                Entregas (Vendedor)
              </div>
              <Badge className="bg-primary text-white text-xs">
                {grupoRecepcion?.codigoPedido}
              </Badge>
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="flex justify-between items-center bg-amber-50 border border-amber-200 p-3 rounded-xl">
              <p className="text-xs text-amber-800">
                A continuación se muestran las prendas que{" "}
                <strong>Bodega ya despachó</strong>. Revisa físicamente tu
                paquete y dale "Check" a lo que te haya llegado.
              </p>
              <Button
                size="sm"
                variant="outline"
                className="text-xs font-bold text-gray-700 bg-white"
                onClick={() => imprimirActaRecepcionPDF(grupoRecepcion)}
              >
            ...
            <div className="space-y-3">
              {grupoRecepcion?.pedidosAsociados?.map((ped: any) => {
                ...
                const prendasDespachadas =
                  ped.detalles?.filter((d: any) => d.guiaDespachoId !== null) ||
                  [];
                ...
                return (
                  ...
                  >
                    <div className="p-3.5 flex justify-between items-center bg-gray-50/80">
                      ...
                      >
                        <Eye size={14} />{" "}
                        {isExpanded ? "Ocultar" : "Revisar Paquete"}
                      </Button>
                    </div>
                    {isExpanded && (
                      <div className="p-4 border-t border-gray-200 bg-gray-50/30 overflow-x-auto">
                        <table className="w-full text-left text-xs border-collapse min-w-650px">
                          <thead>
                            <tr className="text-gray-500 border-b border-gray-200 font-bold uppercase">
                              ...
                              <th className="pb-2 text-center text-rose-600 w-16">
                                Soporte
                              </th>
                              {esModoAdmin && (
                                <th className="pb-2 text-center text-red-600">
                                  Admin
                                </th>
                              )}
                            </tr>
                                ...
                                >
                                  <td className="py-3 text-center">
                                    <input
                                      ...
                                      checked={
                                        recibido ||
                                        prendasSeleccionadas.includes(p.id)
                                      }
                                      onChange={() =>
                                        toggleRecepcionPrenda(p.id)
                                      }
                                    />
                                  </td>
                                  <td className="py-3">
                                  ...
                                  <td className="py-3 text-center border-l border-gray-100">
                                    <Button
                                      size="icon"
                                      variant="ghost"
                                      title="Reportar Problema con esta Prenda"
                                      className="h-8 w-8 text-rose-500 hover:bg-rose-50 border border-rose-200 shadow-sm"
                                      onClick={() =>
                                        handleAbrirTicketContextual(
                                          p,
                                          ped,
                                          grupoRecepcion,
                                        )
                                    ...
                                    <td className="py-3 text-center">
                                      {recibido && (
                                        <Button
                                          size="icon"
                                          variant="ghost"
                                          title="Desbloquear Recepción"
                                          className="h-7 w-7 text-red-500 hover:bg-red-50 border border-red-200"
                                          onClick={() =>
                                            handleDesbloquearRecepcion(p.id)
                                          }
                                        >
                                          <LockOpen size={12} />
                                        </Button>
          ...
          <DialogFooter className="mt-4 flex justify-between items-center border-t pt-4">
            ...
            {prendasSeleccionadas.length > 0 && (
              ...
              >
                ...
                {saving
                  ? "Firmando..."
                  : `Confirmar Recepción de ${prendasSeleccionadas.length} prenda(s)`}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={modalEditOpen} onOpenChange={setModalEditOpen}>
        <DialogContent className="sm:max-w-6xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[95vh]">
          ...
          <div className="bg-blue-50 border border-blue-200 p-3 rounded-xl flex items-center justify-between mt-4">
            <div>
              <Label className="text-xs font-black text-blue-900 uppercase">
                Fecha de Entrega Prometida (Aplica a toda la escuela) *
              </Label>
              <p className="text-[10px] text-blue-700">
                Esta fecha se enviará a Operaciones para todos los contratos.
              </p>
            </div>
            <Input
              type="date"
              className={`h-9 text-xs font-bold bg-white w-40 ${!globalFechaRequerida ? "border-red-400 border-2" : "border-blue-300"}`}
              value={globalFechaRequerida}
          ...
          <div className="flex flex-col lg:flex-row gap-6 mt-4">
            ...
            <div className="w-full lg:w-2/3 space-y-5">
              <ContratoVentaForm
                ...
                onToggleAdmin={() =>
                  setMostrarFormContrato(!mostrarFormContrato)
                }
              />
              <Button
                size="sm"
                className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold h-11 rounded-xl shadow-md text-sm mt-4"
                disabled={saving}
      ...
      <Dialog open={modalTicketOpen} onOpenChange={setModalTicketOpen}>
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-black text-rose-600 border-b pb-3 flex items-center gap-2">
              <TicketPlus size={20} /> Reportar Problema de Prenda
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="bg-rose-50 border border-rose-100 p-3 rounded-xl text-xs">
              <span className="font-bold text-gray-500 block uppercase mb-1">
                Prenda Seleccionada:
              </span>
              <p className="font-black text-rose-900">
                {prendaParaTicket?.sku} - {prendaParaTicket?.tipoRopa} (
                {prendaParaTicket?.color})
              </p>
              <p className="text-rose-700">
                Contrato #{prendaParaTicket?.contrato} | Talla Actual:{" "}
                <b>{prendaParaTicket?.tallaAnterior}</b>
              </p>
            </div>
            ...
            {ticketData.motivo === "Cambio de Talla" && (
              <div className="space-y-1.5 animate-in fade-in zoom-in">
                <Label className="text-[10px] font-bold text-gray-500 uppercase">
                  Nueva Talla Requerida *
                </Label>
                <Input
                  className="h-9 text-xs font-black border-rose-300 focus:border-rose-500 bg-rose-50/50"
                  placeholder="Ej: L, XL, 34..."
                  value={ticketData.nuevaTalla}
                  onChange={(e) =>
                    setTicketData({ ...ticketData, nuevaTalla: e.target.value })
                  }
                />
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold text-gray-500 uppercase">
                Detalle Adicional / Explicación *
              </Label>
              <textarea
                className="w-full h-20 border border-gray-300 rounded-lg p-2 text-xs resize-none outline-none focus:border-primary"
                placeholder="Explica brevemente por qué devuelves la prenda..."
                value={ticketData.mensaje}
                onChange={(e) =>
                  setTicketData({ ...ticketData, mensaje: e.target.value })
                }
              />
            </div>
          </div>
          <DialogFooter className="mt-2 border-t pt-4 flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setModalTicketOpen(false)}
              className="h-9"
            >
              Cancelar
            </Button>
            <Button
              size="sm"
              className="bg-rose-600 hover:bg-rose-700 text-white font-bold h-9 shadow-md"
              disabled={saving}

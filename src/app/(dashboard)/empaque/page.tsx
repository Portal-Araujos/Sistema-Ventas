...
function EmpaquePageContent() {
  ...
  const getSortIcon = (columnName: string) => {
    if (sortConfig.key !== columnName)
      return <ArrowUpDown size={14} className="text-gray-400 shrink-0" />;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, selectedInstFilter, kpiFilter]);
  ...
  const handleOpenEmpacar = (contrato: any) => {
    ...
    setResponsable(
      contrato.responsableEmpaque !== "Sin Asignar"
        ? contrato.responsableEmpaque
        : "",
    );
    const prendasSeguras =
      contrato.detallesCompletos || contrato.detalles || [];
  ...
  const handleCambiarEstadoPrenda = (id: string, nuevoEstado: string) => {
    setPrendasChecklist((prev) =>
      prev.map((p) => {
        if (p.id === id) {
          const nuevasEscaneadas = nuevoEstado === "Preparado" ? p.cantidad : 0;
          return {
            ...p,
            estadoEmpaque: nuevoEstado,
            escaneadas: nuevasEscaneadas,
          };
        }
        return p;
      }),
  ...
  const procesarEscaneo = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      for (let i = 0; i < checklistActualizada.length; i++) {
        ...
        const llegoAEmpaque =
          estOp.includes("empaque") ||
          estOp.includes("listos") ||
          estOp.includes("despacho");
        if (
          p.skuCodigo?.toUpperCase() === sku &&
          llegoAEmpaque &&
          p.guiaDespachoId === null
        ) {
  ...
  const handleGuardarChecklist = async () => {
    setSaving(true);
    try {
      const actualizacionesPermitidas = prendasChecklist
        .filter((p) => {
          const estOp = (p.estadoOperacion || "").toLowerCase();
          const llegoAEmpaque =
            estOp.includes("empaque") ||
  ...
  const iniciarGeneracionGuiaGlobal = () => {
    ...
    if (preparadas.length === 0) {
      showToast(
        "error",
        'No hay prendas en estado "Preparado" en este pedido.',
      );
      return;
    }
  ...
  const exportarMasterChecklistExcel = (grupo: any) => {
    ...
    if (prendasAImprimir.length === 0)
      return showToast("error", "No hay prendas para exportar.");
  ...
  const getEstadoColor = (estado: string) => {
    ...
    if (e.includes("preparado") || e.includes("listos"))
      return "bg-teal-100 text-teal-800 border-teal-200";
    if (e.includes("despacho") || e.includes("completado"))
  ...
  const filteredData = data.filter((g) => {
    const matchSearch =
      ...
      g.codigoOP?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    if (kpiFilter === "Pendiente")
      matchKpi =
        g.estadoGlobal !== "Completado" && g.estadoGlobal !== "En Preparación";
    if (kpiFilter === "En Preparación")
      matchKpi = g.estadoGlobal === "En Preparación";
      ...
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === "fechaRequeridaTexto") {
          const parseDate = (dStr: string) => {
            if (!dStr || dStr.includes("1969") || dStr.includes("1970"))
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-gray-50/30">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <Package className="text-purple-600" /> Bodega y Empaque
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Control logístico y generación de Guías de Despacho Globales.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div
          onClick={() =>
            setKpiFilter(kpiFilter === "Pendiente" ? "" : "Pendiente")
          }
          className={`p-5 rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${kpiFilter === "Pendiente" ? "bg-gray-800 text-white shadow-lg" : "bg-white hover:border-gray-400"}`}
        >
          <div>
            <p
              className={`text-xs font-bold uppercase ${kpiFilter === "Pendiente" ? "text-gray-300" : "text-gray-500"}`}
            >
          ...
          <Clock
            size={40}
            className={
              kpiFilter === "Pendiente" ? "text-gray-600" : "text-gray-200"
            }
          />
        </div>
        ...
        <div
          onClick={() =>
            setKpiFilter(kpiFilter === "En Preparación" ? "" : "En Preparación")
          }
          className={`p-5 rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${kpiFilter === "En Preparación" ? "bg-amber-500 text-white shadow-lg" : "bg-white hover:border-amber-400"}`}
        >
          <div>
            <p
              className={`text-xs font-bold uppercase ${kpiFilter === "En Preparación" ? "text-amber-100" : "text-amber-600"}`}
            >
              En Preparación (Armando)
            </p>
            <p className="text-4xl font-black mt-1">
              {kpis.enPreparacion || 0}
            </p>
          </div>
          <Package
            size={40}
            className={
              kpiFilter === "En Preparación"
                ? "text-amber-400"
                : "text-amber-100"
            }
          />
        ...
        <div
          onClick={() =>
            setKpiFilter(kpiFilter === "Completado" ? "" : "Completado")
          }
          className={`p-5 rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${kpiFilter === "Completado" ? "bg-emerald-600 text-white shadow-lg" : "bg-white hover:border-emerald-400"}`}
        >
          <div>
            <p
              className={`text-xs font-bold uppercase ${kpiFilter === "Completado" ? "text-emerald-200" : "text-emerald-600"}`}
            >
              Completados / Listos
            </p>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-3 items-center">
        <div className="relative w-full md:w-1/3">
          ...
          <Input
            ...
            placeholder="Buscar PED- o Institución..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        <select
          className="h-10 border rounded-xl px-3 text-xs font-bold bg-gray-50 w-full md:w-1/3"
              ...
              <thead className="sticky top-0 z-20 bg-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1)] select-none">
                <tr className="text-gray-600 font-black uppercase border-b border-gray-300">
                  <th
                    ...
                    onClick={() => handleSort("codigoOP")}
                  >
                    <div className="flex items-center gap-1">
                      Código Pedido {getSortIcon("codigoOP")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                  ...
                  >
                    <div className="flex items-center justify-center gap-1">
                      Contratos {getSortIcon("paquetesCantidad")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("avanceGlobal")}
                  >
                    <div className="flex items-center gap-1">
                      Avance de Bodega {getSortIcon("avanceGlobal")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("estadoGlobal")}
                  >
                    <div className="flex items-center justify-center gap-1">
                      Estado General {getSortIcon("estadoGlobal")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
              ...
              <tbody className="divide-y divide-gray-100">
                {paginatedData.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-3.5 font-mono font-black text-purple-700">
                        {item.codigoOP}
                      </td>
                      <td className="p-3.5 font-bold text-gray-900 truncate max-w-100">
                      ...
                      <td className="p-3.5 text-center">
                        ...
                        <div className="text-[10px] font-bold text-gray-500 mt-1">
                          Tot: {item.totalPrendasEscuela} | Desp:{" "}
                          {item.despachadasHistoricasEscuela} |{" "}
                          <span className="text-red-500">
                            Saldo: {item.saldoPendienteEscuela}
                          </span>{" "}
                          |{" "}
                          <span className="text-blue-600">
                            Listo: {item.preparadasSinDespacharEscuela}
                          </span>
                        </div>
                      </td>
                      <td className="p-3.5">
                        <div className="flex items-center gap-2">
                          <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                            <div
                              className={`h-2.5 rounded-full ${getBarColor(item.avanceGlobal)}`}
                              style={{ width: `${item.avanceGlobal}%` }}
                            ></div>
                          </div>
                        ...
                        <div className="text-[9px] text-gray-500 mt-1">
                          {item.preparadasSinDespacharEscuela +
                            item.despachadasHistoricasEscuela}{" "}
                          de {item.totalPrendasEscuela} prendas
                          listas/despachadas
                        </div>
                      </td>
                      <td className="p-3.5 text-center">
                        <Badge
                          className={
                            item.estadoGlobal === "Completado"
                              ? "bg-emerald-100 text-emerald-800 border-emerald-200"
                              : item.estadoGlobal === "En Preparación"
                      ...
                      <td className="p-3.5 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Button
                            size="icon"
                            variant="ghost"
                            title="Imprimir Global (PDF)"
                            className="h-8 w-8 text-gray-700 hover:bg-gray-100"
                            onClick={() => imprimirMasterChecklist(item)}
                          >
                            <Printer size={16} />
                          </Button>
                          <Button
                            ...
                            onClick={() => exportarMasterChecklistExcel(item)}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 text-xs font-bold text-primary border-primary/30"
        ...
        <DialogContent className="sm:max-w-5xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900 border-b pb-3 flex justify-between items-center">
              <span>
                Auditoría de Despachos: {grupoDetalle?.institucionNombre}
              </span>
              <Badge className="bg-purple-600 text-white">
                {grupoDetalle?.codigoOP}
              </Badge>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-2">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-300 shadow-inner text-xs">
              ...
              <div className="flex flex-col">
                ...
                <span className="font-black text-gray-800 text-xl">
                  {grupoDetalle?.totalPrendasEscuela}{" "}
              ...
              <div className="flex flex-col">
                ...
                <span className="font-black text-emerald-600 text-xl">
                  {grupoDetalle?.despachadasHistoricasEscuela}{" "}
                  <span className="text-[10px] font-normal text-gray-500">
                    prendas
                  </span>
                </span>
              ...
              <div className="flex flex-col">
                ...
                <span className="font-black text-red-600 text-xl">
                  {grupoDetalle?.saldoPendienteEscuela}{" "}
                  <span className="text-[10px] font-normal text-gray-500">
                    prendas
                  </span>
                </span>
              ...
              <div className="flex flex-col">
                ...
                <span className="font-black text-blue-600 text-xl">
                  {grupoDetalle?.preparadasSinDespacharEscuela}{" "}
                  <span className="text-[10px] font-normal text-gray-500">
                    prendas
                  </span>
                </span>
            ...
            {grupoDetalle?.preparadasSinDespacharEscuela > 0 && (
              <Button
                size="lg"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white shadow-lg font-black text-sm h-12"
                onClick={iniciarGeneracionGuiaGlobal}
              >
                <Send size={18} className="mr-2" /> GENERAR GUÍA DE DESPACHO
                GLOBAL ({grupoDetalle?.preparadasSinDespacharEscuela} prendas
                listas)
              </Button>
            )}

            <p className="text-xs font-black uppercase text-gray-500 border-b pb-1 mt-4">
              Lista de Contratos para Empacar (Checklist):
            </p>
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <div className="text-[10px] font-bold text-gray-500">
                            Tot: {ped.totalPrendasContrato} | Desp:{" "}
                            {ped.despachadasHistoricasContrato} |{" "}
                            <span className="text-red-500">
                              Saldo: {ped.saldoPendienteContrato}
                            </span>{" "}
                            |{" "}
                            <span className="text-blue-600">
                              Listo: {ped.preparadasSinDespacharContrato}
                            </span>
                          </div>
                        </td>
                        <td className="p-3 text-gray-600 font-semibold">
                          {ped.responsableEmpaque}
                        </td>
                        <td className="p-3 text-center">
                          <Button
                            size="sm"
                            variant={finalizado ? "outline" : "secondary"}
                            className={`text-xs font-bold h-8 ${finalizado ? "border-emerald-300 text-emerald-700" : "bg-primary text-white hover:bg-primary/90"}`}
                            onClick={() => handleOpenEmpacar(ped)}
                          >
                            {finalizado
                              ? "Ver Cerrado"
                              : "📦 Empacar (Checklist)"}
      ...
      <Dialog open={modalEmpacarOpen} onOpenChange={setModalEmpacarOpen}>
        <DialogContent className="sm:max-w-6xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900 border-b pb-2 flex items-center gap-2">
              <Package className="text-amber-600" /> Checklist de Empaque
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-3">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
              <div>
                ...
                <p className="text-sm font-black text-gray-900">
                  {contratoSel?.nombreCliente}
                </p>
                <Badge className="mt-1 bg-blue-100 text-blue-800">
                  Contrato #{contratoSel?.numContrato}
                </Badge>
              </div>
              <div>
                ...
                <Input
                  className="h-9 text-xs mt-1 border-gray-300 bg-white"
            ...
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200 mt-2 flex flex-col sm:flex-row items-center gap-4 shadow-inner">
              ...
              <div className="flex-1 w-full">
                <Label className="text-xs font-black text-emerald-800 uppercase tracking-wide">
                  🔫 Pistoleo Rápido (Lector USB)
                </Label>
                <Input
              ...
              <div className="w-full sm:w-1/3 text-[10px] text-emerald-700 leading-tight font-medium bg-emerald-100/50 p-2 rounded">
                * Cada "BEEP" sumará <b>1 unidad</b>. Cuando el contador alcance
                la cantidad solicitada, la fila se marcará como <b>Preparada</b>{" "}
                automáticamente.
              </div>
            </div>
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          ...
                          {!aunEnTaller && !yaDespachado && (
                            <Badge
                              variant="outline"
                              className={`mt-1 text-[10px] font-bold ${p.escaneadas === p.cantidad ? "bg-emerald-100 text-emerald-700 border-emerald-300" : "bg-amber-50 text-amber-700 border-amber-300"}`}
                            >
                        ...
                        <td className="p-3">
                          {yaDespachado ? (
                            <Badge className="bg-blue-600 text-white w-full flex justify-center">
                              <Truck size={12} className="mr-1" /> Despachado
                            </Badge>
                          ) : aunEnTaller ? (
                            <Badge
                              variant="outline"
                              className="bg-gray-200 text-gray-600 w-full flex justify-center"
                            >
                              <Lock size={12} className="mr-1" /> En Taller /
                              Prod.
                            </Badge>
                          ) : (
                            <select
                              ...
                              onChange={(e) =>
                                handleCambiarEstadoPrenda(p.id, e.target.value)
                              }
                            >
            ...
            <p className="text-[10px] text-gray-500 italic">
              * Solo las prendas que han llegado físicamente a Bodega (Empaque)
          ...
          <DialogFooter className="mt-5 flex gap-2 justify-end">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setModalEmpacarOpen(false)}
            >
              Cancelar
            </Button>
            <Button
        ...
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
            ...
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
              <p className="text-xs text-blue-800 font-medium">
                ...
                <strong className="text-lg font-black">
                  {prendasParaGuia.length}
                </strong>{" "}
                prendas que ya están Listas del pedido{" "}
                <strong>{grupoDetalle?.codigoOP}</strong>.
              </p>
            </div>

            <div>
              <Label className="text-xs font-bold text-gray-700">
                Número de Guía o Nombre del Transportista *
              </Label>
              <Input
              ...
              <p className="text-[10px] text-gray-500 mt-1">
                Este número quedará grabado en el Historial para auditoría.
              </p>
            </div>
          </div>
          ...
          <DialogFooter className="mt-6 flex gap-2 justify-end">
            <Button
              variant="outline"
              size="sm"
              onClick={() => setModalGuiaOpen(false)}
            >
              Cancelar
            </Button>
            <Button
              ...
              disabled={saving}
              onClick={confirmarDespachoGlobal}
            >
              <Send size={14} className="mr-1" />{" "}
              {saving ? "Despachando..." : "Confirmar Salida"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

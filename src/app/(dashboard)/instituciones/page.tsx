...
import { InstitutionsFilters, FiltrosAvanzadosState } from '@/app/features/institutions/components/InstitutionsFilters';
...
export default function InstitucionesPage() {
  ...
  const [filtros, setFiltros] = useState<FiltrosAvanzadosState>(estadoInicialFiltros);

  useEffect(() => {
    fetch('/api/catalogos')
      .then(res => res.json())
      .then(data => { if (data.userRol) setUserRol(data.userRol); });
  }, []);
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Gestión de Instituciones</h1>

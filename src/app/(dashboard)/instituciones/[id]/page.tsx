...
export default function FichaTecnicaPage({
  ...
  const handleEliminar = async () => {
    if (
      !confirm(
        `¿Estás seguro de eliminar "${inst.nombre}"? Se perderá todo su historial.`,
      )
    )
      return;
    try {
      const res = await fetch(`/api/instituciones/${resolvedParams.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Error al eliminar");
      router.push("/instituciones");
    } catch (e) {
      showToast("error", "Error al eliminar institución.");
    }
  };
  ...
  if (!inst)
    return (
      <div className="p-12 text-center text-primary font-bold text-subtitle">
  ...
  if (inst.visitas) {
    inst.visitas.forEach((v: any) => {
      ...
      const resumen = String(
        v.resumenAcuerdos || v.resumen || "",
      ).toLowerCase();
      const isAsignacion =
        tipo.includes("asignación") ||
        tipo.includes("asignacion") ||
        resumen.includes("asignación masiva") ||
        resumen.includes("asignacion masiva");

      // Solo agregamos al historial si NO es una asignación masiva
      if (!isAsignacion) {
        historial.push({
          ...v,
          tipoHistorial: "visita",
          fechaReal: new Date(v.createdAt),
        });
      }
    });
  }
  ...
  if (inst.ventas) {
    inst.ventas.forEach((v: any) =>
      historial.push({
        ...v,
        tipoHistorial: "venta",
        fechaReal: new Date(v.fechaVenta),
      }),
    );
  }
    ...
    <div className="p-4 md:p-8 min-h-screen flex flex-col gap-6 bg-background">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <Button
          ...
          className="text-secondary hover:bg-gray-200 flex items-center gap-2"
        >
        ...
        {esAdmin && (
          <div className="flex gap-2">
            <Button
              ...
              className="text-blue-600 border-blue-200 hover:bg-blue-50 flex items-center gap-1.5 font-bold"
            >
            ...
            <Button
              ...
              className="text-primary border-primary/20 hover:bg-primary/5 flex items-center gap-1.5 font-bold"
            >
      ...
      <div className="bg-card p-6 md:p-8 rounded-2xl border border-border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <div className="flex items-center gap-2 flex-wrap mb-3">
            <Badge className="bg-foreground text-background text-xs px-3 py-1 font-semibold">
              {inst.tamano}
            </Badge>
            <Badge className="bg-primary/10 text-primary text-xs px-3 py-1 font-bold border border-primary/20">
              {inst.estadoComercial}
            </Badge>
          </div>
          <p className="text-normal text-secondary flex items-center gap-2">
            <MapPin size={16} className="text-primary" />
            {inst.parroquia?.canton?.provincia?.nombre} /{" "}
            {inst.parroquia?.canton?.nombre} / Parr. {inst.parroquia?.nombre}
          </p>
        </div>
        <div className="bg-muted p-5 rounded-xl border border-border min-w-260px">
          <span className="text-secondary text-[11px] font-bold block uppercase tracking-wider mb-2">
          ...
          {inst.vendedor ? (
            ...
            <p className="text-normal font-bold text-amber-600 flex items-center gap-2">
              <AlertCircle size={16} /> Sin Asignar
            </p>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
            <h3 className="text-card-title border-b border-border pb-3 flex items-center gap-2 mb-4">
              <Building2 className="text-primary" size={20} /> Datos de la
              Institución
            </h3>
            <div className="space-y-3">
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-secondary">Sostenimiento</span>
                <span className="text-normal font-semibold">
                  {inst.sostenimiento?.nombre}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-secondary">Jornada</span>
                <span className="text-normal font-semibold">
                  {inst.jornada?.nombre}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                ...
                <span className="text-normal font-semibold">
                  {inst.nivelEducativo?.nombre || "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-secondary">Régimen</span>
                <span className="text-normal font-semibold">
                  {inst.regimen?.nombre || "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-secondary">Modalidad</span>
                <span className="text-normal font-semibold">
                  {inst.modalidad?.nombre || "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-secondary">Área</span>
                <span className="text-normal font-semibold">
                  {inst.area?.nombre || "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-gray-500">Jurisdicción:</span>
                <span className="font-semibold">
                  {inst.jurisdiccion?.nombre || "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Acceso Edificio:</span>
                <span className="font-semibold">
                  {inst.accesoEdificio?.nombre || "N/A"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between border-b pb-1.5">
                ...
                <span className="font-semibold">
                  {inst.usuarioCreador?.nombre || "Sistema"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1.5">
                <span className="text-gray-500">Fecha de creacion:</span>
                <span className="font-semibold">
                  {new Date(inst.createdAt).toLocaleDateString("es-EC")}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-bold text-emerald-600">
                  {historial.filter((h) => h.tipoHistorial === "visita").length}{" "}
                  visitas
                </span>
              </div>
            </div>
          ...
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4 text-center mb-4">
              <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                ...
                <span className="text-subtitle font-bold text-blue-900">
                  {inst.docentesHombres}
                </span>
              </div>
              <div className="bg-pink-50 p-4 rounded-xl border border-pink-100">
                <span className="text-[11px] text-pink-600 font-bold uppercase tracking-wider block">
                  Mujeres
                </span>
                <span className="text-subtitle font-bold text-pink-900">
                  {inst.docentesMujeres}
                </span>
              </div>
            </div>
            <div className="bg-muted p-4 rounded-xl text-center border border-border">
              <span className="text-secondary text-[11px] font-bold uppercase block tracking-wider">
                Total Oficial
              </span>
        ...
        <div className="lg:col-span-2">
          <div className="bg-card p-6 md:p-8 rounded-2xl border border-border shadow-sm h-full flex flex-col">
            <h2 className="text-subtitle mb-6 flex items-center gap-2 border-b border-border pb-4">
              <CalendarCheck className="text-primary" size={24} /> Historial de
              Relación Comercial
            </h2>
            ...
            {historial.length === 0 ? (
              <div className="text-center py-12 flex-1">
                ...
                <p className="text-normal font-bold text-foreground">
                  Sin historial registrado
                </p>
                <p className="text-secondary mt-1">
                  Aún no hay visitas ni ventas reales en esta escuela.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {paginatedHistorial.map((item: any) => (
                  <div
                    key={item.id}
                    className="p-4 rounded-xl border border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between gap-4"
                  >
                    {item.tipoHistorial === "visita" ? (
                      <div className="space-y-1 w-full">
                        ...
                        {item.latitud && item.longitud && (
                          <div className="mt-2">
                            <a
                              href={`http://maps.google.com/?q=${item.latitud},${item.longitud}`}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 py-1.5 rounded-lg font-bold items-center gap-1 hover:bg-emerald-100"
                            >
                      ...
                      <div className="space-y-1 w-full">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-gray-700">
                            Contrato N° {item.numContrato}
                          </span>
                          <span className="text-xs text-gray-400">
                            •{" "}
                            {new Date(item.fechaVenta).toLocaleString("es-EC")}
                          </span>
                        </div>
                        <div className="mt-2 bg-emerald-50 p-3.5 rounded-xl border border-emerald-200/80 shadow-2xs">
                          <div className="flex justify-between items-center">
                            ...
                            <Badge
                              variant="outline"
                              className="text-[10px] border-emerald-300 text-emerald-700 bg-white"
                            >
            ...
            {totalPages > 1 && (
              <div className="mt-6 pt-4 border-t border-border flex justify-between items-center">
                <span className="text-xs text-muted-foreground font-medium">
                  ...
                  {Math.min(currentPage * itemsPerPage, historial.length)} de{" "}
                  {historial.length}
                </span>
                <div className="flex gap-2">
                  <Button
        ...
        <DialogContent className="sm:max-w-2xl bg-card p-6 rounded-xl overflow-y-auto max-h-[85vh]">
          ...
          <div className="space-y-4 mt-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="col-span-2">
                <Label className="text-xs font-semibold">Área Educativa</Label>
                <select
                  className="w-full h-9 border rounded-md px-2 text-xs bg-white"
                  value={editFormData.areaId || ""}
                  onChange={(e) =>
                    setEditFormData({ ...editFormData, areaId: e.target.value })
                  }
                >
                  <option value="">Seleccione...</option>
      ...
      {toastMsg && (
        <div
          className={`fixed bottom-6 right-6 z-9999 px-5 py-3.5 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-8 fade-in duration-300 ${toastMsg.tipo === "exito" ? "bg-emerald-600 text-white" : "bg-red-600 text-white"}`}
        >
          {toastMsg.tipo === "exito" ? (

...
function OperacionesPageContent() {
  ...
  const [contratoExpandido, setContratoExpandido] = useState<string | null>(
    null,
  );
  ...
  const abrirModalFechaMasiva = () => {
    ...
    setNuevaFechaReq(
      primerPedido?.fechaRequerida
        ? new Date(primerPedido.fechaRequerida).toISOString().split("T")[0]
        : "",
    );
    setMotivoFechaReq("");
  ...
  const handleGuardarFechaReq = async () => {
    ...
    if (motivoFechaReq.trim().length < 10)
      return showToast(
        "error",
        "Justificación muy corta. Explique el motivo (mínimo 10 letras).",
      );
    ...
    try {
      const res = await fetch("/api/operaciones", {
        ...
        body: JSON.stringify({
          modo: "cambiar_fecha_requerida",
          pedidoIds: pedidoIdsReq,
          nuevaFecha: nuevaFechaReq,
          motivo: motivoFechaReq,
        }),
      });
      ...
      showToast(
        "exito",
        "Fechas de entrega reprogramadas en todos los contratos.",
      );
  ...
  const cargarDatos = async () => {
    ...
    try {
      const [resOps, resInst] = await Promise.all([
        fetch(
          `/api/operaciones?estado=${selectedEstadoFilter}&fechaInicio=${fechaDesde}&fechaFin=${fechaHasta}`,
        ),
      ...
      setInstitucionesList(
        Array.isArray(jsonInst) ? jsonInst : jsonInst.data || [],
      );
    } catch (e) {
  ...
  useEffect(() => {
    cargarDatos();
  }, [selectedEstadoFilter, fechaDesde, fechaHasta]);
  ...
  const handleOpenStock = (grupo: any) => {
    ...
    grupo.pedidosAsociados.forEach((ped: any) => {
      ped.detalles.forEach((det: any) => {
        ...
        const tieneObservacion =
          det.observacion && det.observacion.trim() !== "";
    ...
    if (arrayStock.length === 0) {
      return showToast(
        "error",
        'No hay prendas en "Pendiente en revisión" para balancear.',
      );
    }
    ...
    arrayStock.forEach((item) => {
      initInputs[item.key] = 0;
    });
  ...
  const handleGuardarStock = async () => {
    ...
    try {
      ...
      showToast(
        "exito",
        "¡Balance de Stock aplicado a Producción y Despachos!",
      );
  ...
  const abrirModalGestion = (pIds: string[]) => {
    if (pIds.length === 0)
      return showToast("error", "Seleccione al menos una prenda.");
    setPrendasSeleccionadas(pIds);
    setFechaConfeccionSel("");
    setObservacionOpSel("");
  ...
  const handleGuardarGestion = async () => {
    if (!nuevoEstadoSel)
      return showToast("error", "Seleccione un estado del catálogo.");
    setSaving(true);
    try {
      const res = await fetch("/api/operaciones", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prendaIds: prendasSeleccionadas,
          nuevoEstado: nuevoEstadoSel,
          fechaEstimadaConfeccion: fechaConfeccionSel || undefined,
          observacionOperaciones: observacionOpSel,
        }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error);
  ...
  const getEstadoColor = (estado: string) => {
    ...
    if (e.includes("varios"))
      return "bg-indigo-100 text-indigo-800 border-indigo-300";
    if (e.includes("revision"))
      return "bg-amber-100 text-amber-800 border-amber-200";
    if (e.includes("producci"))
      return "bg-purple-100 text-purple-800 border-purple-200";
    if (e.includes("empaque"))
      return "bg-blue-100 text-blue-800 border-blue-200";
    if (e.includes("listos"))
  ...
  const filteredData = data.filter((g) => {
    ...
    const matchInst = selectedInstFilter
      ? g.institucionId === selectedInstFilter
      : true;
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortConfig) {
      ...
      if (
        ...
        const parseDate = (dStr: string) => {
          if (
            !dStr ||
            dStr.includes("No asig") ||
            dStr.includes("Sin Asig") ||
            dStr.includes("1969") ||
            dStr.includes("1970")
          )
    ...
    const isFantasmaA =
      !a.fechaRequeridaTexto ||
      a.fechaRequeridaTexto.includes("1969") ||
      a.fechaRequeridaTexto.includes("1970");
    const isFantasmaB =
      !b.fechaRequeridaTexto ||
      b.fechaRequeridaTexto.includes("1969") ||
      b.fechaRequeridaTexto.includes("1970");
    ...
    return (
      parseDateDefault(a.fechaRequerida || a.fechaRequeridaTexto) -
      parseDateDefault(b.fechaRequerida || b.fechaRequeridaTexto)
    );
  });
  ...
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-gray-50/30">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Control de Producción y Asignación Inteligente de Stock.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        {/* Tus KPIs... */}
        <button
          onClick={() => setSelectedEstadoFilter("PENDIENTES")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "PENDIENTES" ? "bg-slate-800 text-white border-slate-900 shadow-md" : "bg-white border-gray-200 hover:border-slate-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "PENDIENTES" ? "text-white" : "text-gray-500"}`}
          >
            Total Pendientes
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("Pendiente en revision")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "Pendiente en revision" ? "bg-amber-500 text-white border-amber-600 shadow-md" : "bg-white border-gray-200 hover:border-amber-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "Pendiente en revision" ? "text-white" : "text-gray-500"}`}
          >
            En Revisión
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("En produccion")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "En produccion" ? "bg-purple-600 text-white border-purple-700 shadow-md" : "bg-white border-gray-200 hover:border-purple-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "En produccion" ? "text-white" : "text-gray-500"}`}
          >
            En Producción
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("en empaque")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "en empaque" ? "bg-blue-600 text-white border-blue-700 shadow-md" : "bg-white border-gray-200 hover:border-blue-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "en empaque" ? "text-white" : "text-gray-500"}`}
          >
            En Empaque
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("Listos para el despacho")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "Listos para el despacho" ? "bg-teal-600 text-white border-teal-700 shadow-md" : "bg-white border-gray-200 hover:border-teal-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "Listos para el despacho" ? "text-white" : "text-gray-500"}`}
          >
            Listos Despacho
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("HOY")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "HOY" ? "bg-emerald-600 text-white border-emerald-700 shadow-md" : "bg-white border-gray-200 hover:border-emerald-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "HOY" ? "text-white" : "text-gray-500"}`}
          >
            Despachos Hoy
          </p>
        ...
        <button
          onClick={() => setSelectedEstadoFilter("ATRASADOS")}
          className={`p-3 rounded-xl border text-left transition-all ${selectedEstadoFilter === "ATRASADOS" ? "bg-red-600 text-white border-red-700 shadow-md" : "bg-red-50 border-red-200 hover:border-red-300"}`}
        >
          <p
            className={`text-[10px] font-bold uppercase ${selectedEstadoFilter === "ATRASADOS" ? "text-white" : "text-red-600"}`}
          >
            Atrasados
          </p>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
        <div className="relative">
          ...
          <Input
            className="pl-9 text-xs h-10 bg-gray-50 border-gray-200"
        ...
        <select
          className="h-10 border border-gray-200 rounded-xl px-3 text-xs font-bold bg-gray-50 text-gray-700 w-full outline-none"
        ...
        <div className="flex items-center gap-2">
          ...
          {selectedEstadoFilter !== "TODOS" && (
            <Button
              size="sm"
              variant="ghost"
              className="text-xs text-red-500 font-bold h-8"
        ...
        <div className="flex items-center gap-2 bg-blue-50/50 p-1.5 rounded-xl border border-blue-100">
          ...
          <div className="flex items-center gap-2 w-full">
            <Input
              type="date"
              className="h-8 text-[11px] bg-white border-blue-200 w-full"
              value={fechaDesde}
            ...
            <table className="w-full text-left border-collapse text-xs min-w-800px">
              <thead className="sticky top-0 z-20 bg-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.1)] select-none">
                <tr className="text-gray-600 font-black uppercase border-b border-gray-300">
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("codigoPedido")}
                  >
                    <div className="flex items-center gap-1">
                      Código {getSortIcon("codigoPedido")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                  ...
                  >
                    <div className="flex items-center gap-1">
                      Institución {getSortIcon("institucionNombre")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("vendedorNombre")}
                  >
                    <div className="flex items-center gap-1">
                      Vendedor {getSortIcon("vendedorNombre")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("fechaIngresoTexto")}
                  >
                    <div className="flex items-center gap-1">
                      Fecha Ingreso {getSortIcon("fechaIngresoTexto")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("paquetesCantidad")}
                  >
                    <div className="flex items-center justify-center gap-1">
                      Paquetes {getSortIcon("paquetesCantidad")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
                  ...
                  >
                    <div className="flex items-center justify-center gap-1">
                      Estado Actual {getSortIcon("estadoActual")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("fechaRequeridaTexto")}
                  >
                    <div className="flex items-center justify-center gap-1">
                      Fecha Requerida {getSortIcon("fechaRequeridaTexto")}
                    </div>
                  </th>
                  <th
                    className="p-3.5 text-center cursor-pointer hover:bg-gray-200 transition-colors group"
                    onClick={() => handleSort("fechaEstimadaConfeccionTexto")}
                  >
                    <div className="flex items-center justify-center gap-1">
                      Est. Confección{" "}
                      {getSortIcon("fechaEstimadaConfeccionTexto")}
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-gray-100">
                {paginatedData.map((item) => {
                  const isFantasma =
                    item.fechaRequeridaTexto?.includes("1969") ||
                    item.fechaRequeridaTexto?.includes("1970");
                  const fechaReqCorregida = isFantasma
                    ? "No asignada"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3.5 font-bold text-gray-900 truncate max-w-150px">
                      ...
                      <td className="p-3.5 text-gray-500">
                        {item.fechaIngresoTexto}
                      </td>
                      <td className="p-3.5 text-center font-bold text-gray-800">
                        {item.paquetesCantidad}
                      </td>
                      <td className="p-3.5 text-center">
                        <Badge className={getEstadoColor(item.estadoActual)}>
                          {item.estadoActual}
                        </Badge>
                      </td>
                      <td className="p-3.5 text-center">
                        <span
                          className={`font-bold px-2 py-1 rounded border ${item.esAtrasado ? "bg-red-50 text-red-600 border-red-200" : "bg-gray-50 text-gray-700 border-gray-200"}`}
                        >
                          {fechaReqCorregida}
                        </span>
                      </td>
                      ...
                      <td className="p-3.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <Button
                            size="icon"
                            variant="ghost"
                            title="Balance de Stock / Producción"
                            className="h-8 w-8 text-amber-600 hover:bg-amber-50 border border-transparent hover:border-amber-200"
                          ...
                          <Button
                            size="icon"
                            variant="ghost"
                            title="Gestionar Prendas Manualmente"
                            className="h-8 w-8 text-blue-600 hover:bg-blue-50 border border-transparent hover:border-blue-200"
        ...
        <DialogContent className="sm:max-w-4xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[85vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900 border-b pb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Box className="text-amber-600" /> Consolidación de Bodega /
                Producción
              </div>
              <Badge className="bg-primary text-white text-xs">
                {grupoStock?.codigoPedido}
              </Badge>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-2">
            <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-2">
              <h3 className="text-sm font-black text-blue-900 uppercase">
                Institución: {grupoStock?.institucionNombre}
              </h3>
              <p className="text-xs text-blue-700 mt-1">
                Este panel agrupa prendas idénticas de todos los contratos
                "Pendientes de Revisión". Ingresa lo que tengas en bodega física
                y el sistema dividirá o desatascará los contratos
                automáticamente.
              </p>
            </div>
            ...
            {prendasCustomCount > 0 && (
              <div className="bg-purple-50 p-3 rounded-lg border border-purple-200 flex items-center gap-3 mb-4">
                ...
                <p className="text-xs text-purple-800 font-medium">
                  <strong>Atención: {prendasCustomCount} prenda(s)</strong> de
                  esta lista tiene una{" "}
                  <strong className="font-black">Observación Especial</strong>.
                  La tabla de abajo te muestra el <strong>Total General</strong>
                  , pero al procesar, el sistema enviará obligatoriamente la
                  prenda con observación a Producción, y usará tu Stock para
                  asignarlo únicamente a las prendas regulares.
                </p>
              </div>
            )}
            <div className="bg-purple-50 p-4 rounded-xl border border-purple-200 mb-2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <Label className="text-xs font-black text-purple-900 uppercase">
                  Fecha Est. Confección (Para Taller)
                </Label>
                <p className="text-[10px] text-purple-700 mt-0.5">
                  Asigna el plazo de entrega a las prendas que se enviarán a
                  Producción por falta de stock.
                </p>
              </div>
              <Input
                type="date"
            ...
            {resumenStock.length > 0 ? (
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-gray-100 border-b border-gray-200 text-gray-600 uppercase">
                      ...
                      <th className="p-3 text-center font-bold bg-amber-50/50 border-l border-amber-100 w-32">
                        En Stock
                      </th>
                      <th className="p-3 text-center font-bold text-red-600 bg-red-50/50">
                        A Producción
                      </th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={row.key} className="hover:bg-gray-50">
                          ...
                          <td className="p-3 text-center text-gray-600">
                            {row.talla} ({row.color}) ({row.genero})
                          </td>
                          <td className="p-3 text-[10px]">
                            <div className="font-bold text-purple-700">
                              {row.bordado || "Sin bordado"}
                            </div>
                            <div className="text-gray-500 italic mt-0.5">
                              {row.observacion || "Sin obs."}
                            </div>
                          </td>
                          <td className="p-3 text-center font-black text-gray-900 text-sm">
                            {row.totalSolicitado}
                          </td>
                          <td className="p-2 border-l border-amber-50 bg-amber-50/30 text-center">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setStockInputs({
                                  ...stockInputs,
                                  [row.key]: Math.min(row.totalSolicitado, val),
                                });
                              }}
                            />
                          </td>
              ...
              <p className="text-center text-sm font-bold text-gray-400 py-6">
                No hay prendas genéricas pendientes.
              </p>
            )}
          </div>
          ...
          <DialogFooter className="mt-5 flex justify-between items-center border-t pt-4">
            ...
            <Button
              size="sm"
              className="bg-slate-900 hover:bg-slate-800 text-white font-bold h-10 px-6 shadow-md"
              disabled={saving}
              onClick={handleGuardarStock}
            >
              {saving
                ? "Calculando y Procesando..."
                : "⚙️ Procesar y Asignar (Desatascar)"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={modalDetalleOpen} onOpenChange={setModalDetalleOpen}>
        <DialogContent className="sm:max-w-6xl bg-white p-6 rounded-2xl overflow-y-auto max-h-[88vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-gray-900 border-b pb-3 flex items-center justify-between">
              <span>
                Control Operativo Manual - {grupoDetalle?.institucionNombre}
              </span>
              <Badge className="bg-primary text-white text-xs">
                {grupoDetalle?.codigoPedido}
              </Badge>
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border text-xs">
              ...
              <div>
                ...
                <span className="font-extrabold text-gray-800">
                  {grupoDetalle?.institucionNombre}
                </span>
              </div>
              <div>
                <span className="text-gray-400 block font-bold uppercase">
            ...
            <div className="mt-3 mb-2 bg-blue-50 border border-blue-200 p-3 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                <p className="text-[10px] font-bold text-gray-500 uppercase flex items-center gap-1">
                  <Calendar size={12} /> Entrega Prometida Global:
                </p>
                <p
                  className={`text-sm font-black ${grupoDetalle?.esAtrasado ? "text-red-600" : "text-gray-900"}`}
                >
              ...
              <Button
                size="sm"
                variant="outline"
                className="text-blue-700 bg-white border-blue-300 hover:bg-blue-100 font-bold shadow-sm"
                onClick={abrirModalFechaMasiva}
              >
                <Edit2 size={14} className="mr-2" /> Reprogramar Entrega
                Completa
              </Button>
            </div>
      ...
      <Dialog open={modalGestionOpen} onOpenChange={setModalGestionOpen}>
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-black text-gray-900 border-b pb-2">
              Actualizar Operaciones
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <p className="text-xs text-blue-800 bg-blue-50 p-2 rounded border border-blue-100 font-medium">
              Aplicando cambios a <strong>{prendasSeleccionadas.length}</strong>{" "}
              prenda(s).
            </p>
            <div>
              ...
              <select
                className="w-full h-10 border rounded-xl px-3 text-sm font-bold bg-white mt-1 outline-none"
                value={nuevoEstadoSel}
                onChange={(e) => setNuevoEstadoSel(e.target.value)}
              >
            ...
            <div>
              <Label className="text-xs font-bold text-gray-700">
                Fecha Estimada de Confección
              </Label>
              <Input
                type="date"
                className="h-10 text-xs font-bold bg-white mt-1"
                value={fechaConfeccionSel}
                onChange={(e) => setFechaConfeccionSel(e.target.value)}
              />
            </div>
            <div>
              <Label className="text-xs font-bold text-gray-700">
              ...
              <Input
                className="h-10 text-xs bg-white mt-1"
                placeholder="Ej: Se envía parcial por tela..."
                value={observacionOpSel}
                onChange={(e) => setObservacionOpSel(e.target.value)}
              />
            </div>
          </div>
        ...
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          ...
          <div className="space-y-4 mt-2">
            <div className="bg-red-50 p-3 rounded-lg border border-red-100 text-xs text-red-800 font-medium leading-relaxed">
              Al cambiar la fecha, el vendedor visualizará este cambio en su
              panel. <br />
              <b>
                Debes justificar el motivo obligatoriamente para fines de
                auditoría.
              </b>
            </div>
            <div>
              <Label className="text-xs font-bold text-gray-700">
                Nueva Fecha de Entrega Prometida *
              </Label>
              <Input
                type="date"
                className="h-10 text-xs font-bold bg-white mt-1 border-gray-300"
                value={nuevaFechaReq}
                onChange={(e) => setNuevaFechaReq(e.target.value)}
              />
            </div>
            <div>
              <Label className="text-xs font-bold text-gray-700">
                Motivo del Cambio / Retraso (Auditoría) *
              </Label>
              <textarea
              ...
              />
              <p
                className={`text-[10px] font-bold mt-1 ${motivoFechaReq.length >= 10 ? "text-emerald-600" : "text-gray-400"}`}
              >
          ...
          <DialogFooter className="mt-5 flex gap-2 justify-end">
            ...
            <Button
              ...
              disabled={saving || motivoFechaReq.trim().length < 10}
              onClick={handleGuardarFechaReq}
            >
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-9999 px-5 py-3.5 rounded-xl shadow-2xl flex items-center gap-3 text-white ${toast.tipo === "exito" ? "bg-emerald-600" : "bg-red-600"}`}
        >
          {toast.tipo === "exito" ? (
    ...
    <Suspense
      fallback={
        <div className="p-16 text-center text-gray-500 font-bold animate-pulse">
          Cargando módulo de operaciones...
        </div>
      }
    >

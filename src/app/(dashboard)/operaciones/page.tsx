...
import { 
  ...
  CheckCircle2, AlertCircle, AlertTriangle, PackageSearch 
} from 'lucide-react';
...
export default function OperacionesPage() {
  ...
  // MODAL DE GESTIÓN RÁPIDA DE ESTADO
  ...
  const handleGuardarGestion = async () => {
    if (!nuevoEstadoSel) {
      showToast('error', 'Seleccione un estado del catálogo.');
      return;
    }
    ...
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen bg-gray-50/30">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Control de Producción y Trazabilidad por Prenda.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {selectedEstadoFilter !== 'TODOS' && (
            <Button size="sm" variant="ghost" className="text-xs text-red-500 font-bold h-8" onClick={() => setSelectedEstadoFilter('TODOS')}>
              Restablecer
            </Button>
          )}
        </div>
      ...
      {/* 🗓️ MODAL: ACTUALIZAR ESTADO Y FECHA */}
        ...
        <DialogContent className="sm:max-w-md bg-white p-6 rounded-2xl">
          ...
          <div className="space-y-4 mt-2">
            <p className="text-xs text-blue-800 bg-blue-50 p-2 rounded border border-blue-100 font-medium">
              Aplicando cambios a <strong>{prendasSeleccionadas.length}</strong> prenda(s).
            </p>
          ...
          <DialogFooter className="mt-5 flex gap-2 justify-end">
            ...
            <Button size="sm" className="bg-primary text-white font-bold" disabled={saving} onClick={handleGuardarGestion}>

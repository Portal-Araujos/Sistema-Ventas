...
export default function IndicadoresPage() {
  ...
  const cargarIndicadores = async () => {
    ...
    try {
      ...
      if (!resInd.ok)
        throw new Error("No se pudo cargar la API de indicadores");
  ...
  const exportarExcel = () => {
    ...
    const dataTransito = reporte.map((r) => {
      const porcentajeRetenido =
        r.metaMonto > 0
          ? ((r.transitoTotal / r.metaMonto) * 100).toFixed(1)
          : "0";
      return {
  ...
  const totalCierreSemanal = reporte.reduce(
    (sum, r) => sum + r.cierreSemanal,
    0,
  );
  const totalMetaSemanal = reporte.reduce((sum, r) => sum + r.metaMonto, 0);
  const totalTransitoSemanal = reporte.reduce(
    (sum, r) => sum + r.transitoTotal,
    0,
  );
  const porcentajeGlobal =
    totalMetaSemanal > 0
      ? ((totalCierreSemanal / totalMetaSemanal) * 100).toFixed(1)
      : "0";
  return (
    <div className="p-4 md:p-8 flex flex-col gap-6 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
            <BarChart3 className="text-emerald-600" /> Indicadores y Cierre
            Semanal
          </h1>
          <p className="text-xs md:text-sm text-gray-500 mt-0.5">
            Mide el desempeño real de tus vendedores (Solo ventas con contratos
        ...
        <div className="flex gap-2">
          ...
          {esAdmin && (
            <Button
              onClick={handleOpenModalMetas}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex gap-2"
            >
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto justify-between md:justify-end">
          <div className="bg-amber-50 p-3 rounded-lg border border-amber-200 text-center min-w-120px">
            ...
            <span className="text-lg font-extrabold text-amber-600">
              ${totalTransitoSemanal.toFixed(2)}
            </span>
          </div>
          ...
          <div className="bg-gray-50 p-3 rounded-lg border text-center min-w-120px">
            ...
            <span
              className={`text-lg font-extrabold ${Number(porcentajeGlobal) >= 100 ? "text-emerald-600" : Number(porcentajeGlobal) >= 50 ? "text-amber-600" : "text-red-600"}`}
            >
              {porcentajeGlobal}%
            </span>
          </div>
      ...
      <div>
        <h2 className="text-sm font-bold text-emerald-800 mb-3 flex items-center gap-2">
          <CheckCircle2 size={16} /> VENTAS VALIDADAS (Suman a Meta)
        </h2>
          ...
          <Table>
            <TableHeader className="bg-emerald-50/80">
              <TableRow>
                ...
                <TableHead className="font-semibold text-red-700 text-center">
                  Domingo
                </TableHead>
                <TableHead className="font-bold text-emerald-800 text-center bg-emerald-100/50">
                  Cierre Real
                </TableHead>
                <TableHead className="font-bold text-gray-800 text-center bg-gray-100/50">
                  Meta Asignada
                </TableHead>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell
                    ...
                    className="text-center py-8 text-gray-500"
                  >
                    No hay vendedores o datos registrados.
                  </TableCell>
                </TableRow>
              ) : (
                reporte.map((r) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.lunes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.martes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.miercoles.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.jueves.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.viernes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-600">
                      ${r.diasReales.sabado.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-red-600 bg-red-50/20">
                      ${r.diasReales.domingo.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-extrabold text-sm text-emerald-700 bg-emerald-50/50">
                    ...
                    <TableCell className="text-center">
                      <Badge
                        className={`text-xs font-bold ${r.porcentajeCumplido >= 100 ? "bg-emerald-100 text-emerald-800 border-emerald-300" : r.porcentajeCumplido >= 50 ? "bg-amber-100 text-amber-800 border-amber-300" : "bg-red-100 text-red-800 border-red-300"}`}
                      >
      ...
      <div className="mt-4 opacity-90">
        <h2 className="text-sm font-bold text-amber-800 mb-3 flex items-center gap-2">
          <AlertTriangle size={16} /> VENTAS PENDIENTES (Falta Facturación /
          Documentos)
        </h2>
          ...
          <Table>
            <TableHeader className="bg-amber-50/80">
              <TableRow>
                ...
                <TableHead className="font-bold text-amber-800 text-center bg-amber-100/50">
                  Total Retenido
                </TableHead>
                <TableHead className="font-bold text-amber-800 text-center bg-amber-100/50">
                  % Retenido
                ...
                reporte.map((r) => (
                  <TableRow key={r.vendedorId} className="hover:bg-amber-50/30">
                    ...
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.lunes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.martes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.miercoles.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.jueves.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.viernes.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-gray-900">
                      ${r.diasTransito.sabado.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-medium text-xs text-red-700 bg-red-50/20">
                      ${r.diasTransito.domingo.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center font-bold text-sm text-amber-700 bg-amber-50/50">
                      ${r.transitoTotal.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge
                        variant="outline"
                        className="text-xs font-bold bg-amber-50 text-amber-800 border-amber-300"
                      >
                        {r.metaMonto > 0
                          ? ((r.transitoTotal / r.metaMonto) * 100).toFixed(1)
                          : "0"}
        ...
        <DialogContent className="sm:max-w-lg bg-white p-6 rounded-2xl max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={handleGuardarMetas} className="space-y-4 mt-2">
            <p className="text-xs text-gray-500">
              Ingresa el objetivo en dólares ($) para cada vendedor en la semana
              elegida ({fechaLunes}):
            </p>
            <div className="space-y-3">
              {metasForm.map((m, idx) => (
                <div
                  key={m.vendedorId}
                  className="flex items-center justify-between gap-3 bg-gray-50 p-3 rounded-lg border"
                >

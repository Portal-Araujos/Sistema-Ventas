  ...
  try {
    ...
    if (payload.rol !== "super_admin" && payload.rol !== "administrador") {
      return NextResponse.json(
        { error: "Solo los administradores pueden armar rutas" },
        { status: 403 },
      );
    }
    const body = await request.json();
    const { institucionIds, vendedorId, fechaProgramada, horaProgramada } =
      body;
    if (
      !institucionIds ||
      ...
      return NextResponse.json(
        { error: "Faltan datos obligatorios para armar la ruta" },
        { status: 400 },
      );
    }
    const escuelasAfectadas = await prisma.institution.findMany({
      ...
      include: {
        ...
        visitas: {
          where: {
            estadoGestion: {
              in: ["Pendiente", "No Visitada", "No visitada", "Reprogramada"],
            },
          },
    ...
    return NextResponse.json(
      { success: true, creadas: result.count },
      { status: 201 },
    );
  } catch (error) {
    console.error("Error Masiva:", error);
    return NextResponse.json(
      { error: "Error interno al generar las rutas" },
      { status: 500 },
    );
  }
}

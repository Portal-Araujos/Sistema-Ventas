  ...
  try {
    ...
    if (usuario && usuario.bloqueado) {
      return NextResponse.json(
        {
          error:
            "Tu cuenta ha sido bloqueada por seguridad tras múltiples intentos fallidos. Contacta al Administrador.",
        },
        { status: 403 },
      );
    }
    ...
    if (!isValid) {
      ...
      if (debeBloquearse) {
        return NextResponse.json(
          {
            error:
              "¡Atención! Has superado los 3 intentos. Tu cuenta ha sido bloqueada.",
          },
          { status: 403 },
        );
      }
      return NextResponse.json(
        {
          error: `Credenciales inválidas. Te quedan ${3 - nuevosFallos} intentos.`,
        },
        { status: 401 },
      );
    }
...
async function castigarIP(
  ip: string,
  nivelBloqueo: number,
  tiempoCastigoMs: number,
) {
  ...
  console.warn(
    `[SEGURIDAD] IP ${ip} bloqueada por nivel ${nivelBloqueo} hasta ${hasta}`,
  );
}

...
export async function GET(request: Request) {
  try {
    ...
    if (userRol === "vendedor") {
      ...
      whereUsuarios.rol = {
        nombre: { in: ["vendedor", "VENDEDOR", "Vendedor"] },
      };
    }
    ...
    const reporte = vendedores.map((vend) => {
      ...
      ventasVend.forEach((v) => {
        const fechaEc = new Date(
          new Date(v.fechaVenta).toLocaleString("en-US", {
      ...
      const cierreSemanal = Object.values(diasReales).reduce(
        (a, b) => a + b,
        0,
      );
      ...
      const porcentajeCumplido =
        metaMonto > 0
          ? parseFloat(((cierreSemanal / metaMonto) * 100).toFixed(1))
          : 0;
      const transitoTotal = Object.values(diasTransito).reduce(
        (a, b) => a + b,
        0,
      );
      return {

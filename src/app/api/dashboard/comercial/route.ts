...
export async function GET(request: Request) {
  try {
    ...
    if (provinciaId)
      whereGlobalInst.parroquia = {
        canton: { provinciaId: parseInt(provinciaId) },
      };
    ...
    const [
      ...
      prisma.visitaAgenda.findMany({
        where: {
          ...whereVisitas,
          estadoGestion: { notIn: ["Pendiente", "No Visitada", "No visitada"] },
        },
      ...
      prisma.visitaAgenda.findMany({
        where: {
          ...
          ...(Object.keys(whereGlobalInst).length > 0
            ? { institucion: whereGlobalInst }
            : {}),
        },
      }),
    ...
    ventas.forEach((v) => {
      ...
      const esValidada =
        v.estadoTicket?.includes("Validado") ||
        (v.verificacionFact &&
          v.verificacionFact !== "Sin validar" &&
          !v.estadoTicket?.includes("Rechazado") &&
          !v.estadoTicket?.includes("Observado"));
      const tieneNovedad =
        v.estadoTicket?.includes("Observado") ||
        v.estadoTicket?.includes("Rechazado") ||
        (v.observacionesFact && v.observacionesFact !== "Sin observaciones");
    ...
    const metaComercial = metasRaw.reduce(
      (acc, m) => acc + (m.montoMeta || 0),
      0,
    );
    const cumplimientoValidado =
      metaComercial > 0 ? (ventasValidadas / metaComercial) * 100 : 0;
    ...
    const pedidosEnviados = pedidos.filter(
      (p) => p.estado !== "Borrador" || p.fechaEnvioOperaciones,
    ).length;
    ...
    pedidos
      .filter((p) => p.estado !== "Despachado" && p.fechaRequerida)
      .forEach((p) => {
    ...
    ventas.forEach((v) => {
      ...
      if (v.tipoCliente?.nombre?.toUpperCase().includes("NUEVA"))
        mapVendedores.get(vId).nuevas += monto;
    });
    ...
    const rankingVendedores = Array.from(mapVendedores.values())
      .map((v) => {
        const porcCumplimiento =
          v.meta > 0 ? (v.ventaValidada / v.meta) * 100 : 0;
        const porcNuevas =
          v.ventaValidada > 0 ? (v.nuevas / v.ventaValidada) * 100 : 0;
        return {
          ...v,
    ...
    return NextResponse.json({
      ...
      origen: {
        ...
        porcNuevas:
          ventasRegistradas > 0
            ? Math.round((origen.nuevas / ventasRegistradas) * 100)
            : 0,
        porcReferidas:
          ventasRegistradas > 0
            ? Math.round((origen.referidas / ventasRegistradas) * 100)
            : 0,
        porcRecompras:
          ventasRegistradas > 0
            ? Math.round((origen.recompras / ventasRegistradas) * 100)
            : 0,
      },
      campo: {
        instAsignadas,
    ...
    return NextResponse.json(
      { error: "Error al generar indicadores" },
      { status: 500 },
    );
  }
}

...
export async function GET(request: Request) {
  try {
    ...
    prendas.forEach(p => {
       ...
       else if (est.includes('producci') || est.includes('corte') || est.includes('confeccion')) enTaller += p.cantidad;
       else if (est.includes('empaque') || est.includes('listos')) enEmpaque += p.cantidad;
    ...
    visitas.forEach(v => {
      ...
      if (!mapVendedores.has(vId)) {
        mapVendedores.set(vId, { nombre: v.usuario?.nombre?.split(' ')[0] || 'Desc.', visitas: 0, contratos: 0, monto: 0 });
      }
    ...
    ventas.forEach(v => {
      ...
      if (!mapVendedores.has(vId)) {
        mapVendedores.set(vId, { nombre: v.vendedor?.nombre?.split(' ')[0] || 'Desc.', visitas: 0, contratos: 0, monto: 0 });
      }
      mapVendedores.get(vId).contratos += 1;
      const num = parseFloat(String(v.valorContrato || '0').replace(/[^0-9.-]+/g,""));
      mapVendedores.get(vId).monto += (isNaN(num) ? 0 : num);
    });

    const topVendedores = Array.from(mapVendedores.values()).sort((a, b) => b.monto - a.monto).slice(0, 5);
    const ultimasVentas = ventas.sort((a, b) => new Date(b.fechaVenta).getTime() - new Date(a.fechaVenta).getTime()).slice(0, 5).map(v => ({
      id: v.id,
      escuela: v.institucion?.nombre || 'Sin Escuela',
      monto: parseFloat(String(v.valorContrato || '0').replace(/[^0-9.-]+/g,"")) || 0,
      vendedor: v.vendedor?.nombre || 'Desc.',
      fecha: new Date(v.fechaVenta).toLocaleDateString('es-EC', {timeZone:'UTC'})
    }));

    const urgentesRaw = await prisma.pedido.findMany({
      where: { estado: { notIn: ['Despachado', 'Borrador'] }, fechaRequerida: { not: null } },
    ...
    const entregasUrgentes = urgentesRaw.map(u => ({
      ...
      fecha: u.fechaRequerida ? new Date(u.fechaRequerida).toLocaleDateString('es-EC', {timeZone:'UTC'}) : '-',
      estado: u.estado
    }));
    ...
    return NextResponse.json({
      userRol,
      kpis: {
        totalMontoVentas, totalContratos, totalVisitas, tasaCierre,
        enRevision, enTaller, enEmpaque, despachadas, prendasAtrasadas
      },
      graficos: {
    ...
    console.error(error);
    return NextResponse.json({ error: 'Error al generar stats' }, { status: 500 });
  }
}
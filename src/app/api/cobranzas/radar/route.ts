...
export async function GET(request: Request) {
  try {
    ...
    const end = fechaHasta ? new Date(`${fechaHasta}T23:59:59.999-05:00`) : new Date(hoy.getTime() + 7 * 24 * 60 * 60 * 1000);
    ...
    const instituciones = await prisma.institution.findMany({
      ...
      include: {
        ...
        tickets: {
          where: { tipo: 'Cobranza', estado: { notIn: ['Cerrado', 'Resuelto'] } }
        }
      },
      orderBy: { nombre: 'asc' }
    });
    ...
    const radarData = instituciones.map(inst => {
      return {
        ...
        vendedorNombre: inst.visitas[0]?.usuario?.nombre || inst.vendedor?.nombre || 'Sin Asignar',
        proximaVisita: inst.visitas.length > 0 ? inst.visitas[0].fechaProgramada : null,
        horaProgramada: inst.visitas.length > 0 ? inst.visitas[0].horaProgramada : null,
    ...
    return NextResponse.json({ error: 'Error al cargar radar' }, { status: 500 });
  }
}

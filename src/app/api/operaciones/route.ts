...
export async function GET(request: Request) {
  try {
    ...
    const [estadosCatalogo, todosLosDetalles, pedidosMaestros] =
      await Promise.all([
    ...
    todosLosDetalles.forEach((det: any) => {
      ...
      const fConfeccion = det.fechaEstimadaConfeccion
        ? new Date(det.fechaEstimadaConfeccion)
        : null;
      const fUpdatedAt = new Date(det.updatedAt);
      const esAtrasado =
        fConfeccion !== null && fConfeccion < hoy && estPrenda !== "Despacho";
      if (estPrenda === "Pendiente en revision")
        kpis.enRevision += det.cantidad;
      if (estPrenda === "En produccion") kpis.enProduccion += det.cantidad;
      if (estPrenda === "en empaque") kpis.enEmpaque += det.cantidad;
      if (estPrenda === "Listos para el despacho")
        kpis.listosDespacho += det.cantidad;
      ...
      if (estadoFiltro !== "TODOS") {
        if (estadoFiltro === "PENDIENTES")
          cumpleFiltro = estPrenda !== "Despacho";
        else if (estadoFiltro === "ATRASADOS") cumpleFiltro = esAtrasado;
        else if (estadoFiltro === "HOY")
          cumpleFiltro =
            estPrenda === "Despacho" &&
            fUpdatedAt >= hoy &&
            fUpdatedAt <= finHoy;
      ...
      if (!mapaEscuelas.has(grupoKey)) {
        ...
        mapaEscuelas.set(grupoKey, {
          ...
          fechaIngresoTexto: new Date(ped.createdAt).toLocaleDateString(
            "es-EC",
            { timeZone: "America/Guayaquil" },
          ),
          fechaRequeridaTexto: ped.fechaRequerida
            ? new Date(ped.fechaRequerida).toLocaleDateString("es-EC", {
                timeZone: "UTC",
              })
            : "No asignada",
          esAtrasado: false,
      ...
      if (fConfeccion) {
        if (
          !escuela.fechaEstimadaConfeccionGlobal ||
          fConfeccion < escuela.fechaEstimadaConfeccionGlobal
        ) {
      ...
      contrato.detalles.push({
        ...
        fechaEstimadaConfeccionTexto: fConfeccion
          ? fConfeccion.toLocaleDateString("es-EC", { timeZone: "UTC" })
          : "Sin Asignar",
      });
    });
    ...
    const tablaRes = Array.from(mapaEscuelas.values()).map((e) => ({
      ...e,
      estadoActual:
        e.estadosUnicosEscuela.size > 1
          ? "Varios Estados"
          : Array.from(e.estadosUnicosEscuela)[0],
      fechaEstimadaConfeccionTexto: e.fechaEstimadaConfeccionGlobal
        ? e.fechaEstimadaConfeccionGlobal.toLocaleDateString("es-EC", {
            timeZone: "UTC",
          })
        : "Sin Asignar",
      pedidosAsociados: Array.from(e.pedidosAsociados.values()).map(
        (c: any) => ({
          ...c,
          estadoGlobalContrato:
            c.estadosUnicosContrato.size > 1
              ? "Varios Estados"
              : Array.from(c.estadosUnicosContrato)[0],
        }),
      ),
    }));
    ...
    return NextResponse.json(
      { error: "Error al consultar operaciones" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (modo === "cambiar_fecha_requerida") {
      ...
      if (!motivo || motivo.trim().length < 10) {
        return NextResponse.json(
          { error: "La justificación es obligatoria (Mínimo 10 letras)." },
          { status: 400 },
        );
      }
      ...
      if (!Array.isArray(pedidoIds) || pedidoIds.length === 0) {
        return NextResponse.json(
          { error: "No hay pedidos para actualizar." },
          { status: 400 },
        );
      }
      ...
      return NextResponse.json({
        success: true,
        message: "Fechas de entrega reprogramadas masivamente.",
      });
    }
    if (modo === "asignacion_stock") {
      ...
      const frStr = institucionId.includes("_")
        ? institucionId.split("_")[1]
        : null;
      ...
      for (const p of prendasPendientes) {
        ...
        if (esPersonalizado) {
          ...
          if (!genericasPorContrato.has(p.pedidoId)) {
            genericasPorContrato.set(p.pedidoId, {
              pedidoId: p.pedidoId,
              totalPrendas: 0,
              items: [],
            });
      ...
      const contratosOrdenados = Array.from(genericasPorContrato.values()).sort(
        (a, b) => a.totalPrendas - b.totalPrendas,
      );
      for (const contrato of contratosOrdenados) {
        for (const prenda of contrato.items) {
          ...
          let stockDisponible = stockAsignado[key]
            ? parseInt(stockAsignado[key])
            : 0;
          ...
          if (stockDisponible >= prenda.cantidad) {
            ...
            if (fechaParseada)
              dataCreate.fechaEstimadaConfeccion = fechaParseada;
      ...
      return NextResponse.json({
        success: true,
        message: "Balance procesado exitosamente.",
      });
    }
    const {
      prendaIds,
      nuevoEstado,
      fechaEstimadaConfeccion,
      observacionOperaciones,
    } = body;

    if (!Array.isArray(prendaIds) || prendaIds.length === 0) {
      return NextResponse.json(
        { error: "Debe seleccionar al menos una prenda." },
        { status: 400 },
      );
    }
    ...
    if (observacionOperaciones !== undefined)
      updateData.observacionOperaciones = observacionOperaciones;
    ...
    return NextResponse.json(
      { error: "Error al actualizar operaciones" },
      { status: 500 },
    );
  }
}

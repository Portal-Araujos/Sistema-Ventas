  ...
  try {
    ...
    if (body.masivo) {
      ...
      for (const row of rows) {
        ...
        const nom = String(
          row.Nombres || row.nombres || row.Nombre || row.nombre || "",
        ).trim();
        ...
        const escText = String(
          row.Escuela || row.escuela || row.Institucion || "",
        ).trim();
        ...
        if (modText) {
          ...
          if (!mod) {
            mod = await prisma.modalidadLaboral.create({
              data: { nombre: modText.toUpperCase() },
            });
          }
          modId = mod.id;
        }

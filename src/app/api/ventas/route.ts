  ...
  try {
    ...
    if (numContratos.length > 0) {
      pedidosAsociados = await prisma.pedido.findMany({
        where: {
          numContrato: { in: numContratos },
          institucionId: { in: instIds },
        },
        select: {
          numContrato: true,
          institucionId: true,
    ...
    const dataFormateada = ventas.map((v) => {
      const ped = pedidosAsociados.find(
        (p) =>
          p.numContrato === v.numContrato &&
          p.institucionId === v.institucionId,
      );
      return {
        id: v.id,
        ...
        provinciaNombre:
          v.institucion.parroquia?.canton?.provincia?.nombre || "",
...
export async function PUT(request: Request) {
  try {
    ...
    if (numContrato !== undefined) {
      ...
      updateData.estadoClienteId = estadoClienteId
        ? parseInt(estadoClienteId)
        : null;
      updateData.estadoContratoId = estadoContratoId
        ? parseInt(estadoContratoId)
        : null;
      ...
      updateData.numeroCedula = numeroCedula
        ? String(numeroCedula).trim()
        : null;
      await prisma.pedido.updateMany({
        where: {
          institucionId: ventaDb.institucionId,
          numContrato: ventaDb.numContrato,
        },
        data: {
    ...
    if (verificacionFact !== undefined) {
      ...
      if (verifLimpia === "") {
        nuevoEstadoTicket = updateData.observacionesFact
          ? "Observado ⚠️ - Pendiente Vendedor"
      ...
      observacionesFact !== undefined &&
      (!ventaDb.verificacionFact || ventaDb.verificacionFact === "Sin validar")
    ) {
      nuevoEstadoTicket = observacionesFact
        ? "Observado ⚠️ - Pendiente Vendedor"
        : "Pendiente Facturación";
    }

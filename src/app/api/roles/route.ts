...
export async function POST(request: Request) {
  try {
    ...
    if (payload.rol !== 'super_admin') return NextResponse.json({ error: 'Solo el Super Admin puede crear roles' }, { status: 403 });
    ...
    const nuevoRol = await prisma.rol.create({
      data: {
        ...
        descripcion,
        permisos: JSON.stringify(permisos || []) // Se guarda como JSON String
      }
    });
...
export async function PUT(request: Request) {
  try {
    ...
    const { payload } = await jwtVerify(token, JWT_SECRET);
    if (payload.rol !== 'super_admin') return NextResponse.json({ error: 'Solo el Super Admin puede editar roles' }, { status: 403 });

  ...
  try {
    ...
    for (const ped of pedidos) {
      ...
      if (ped.fechaRequerida) {
        ...
        if (
          !grupo.fechaRequeridaDate ||
          pedReqDate < grupo.fechaRequeridaDate
        ) {
          grupo.fechaRequeridaDate = pedReqDate;
        }
      }
      const totalContratoReal = ped.detalles.reduce(
        (acc: number, item: any) => acc + (item.cantidad || 1),
        0,
      );
      const despachadasHistoricas = ped.detalles
        .filter((d: any) => d.guiaDespachoId !== null)
        .reduce((acc: number, item: any) => acc + (item.cantidad || 1), 0);
      const preparadasSinDespachar = ped.detalles
        .filter(
          (d: any) =>
            d.estadoEmpaque === "Preparado" && d.guiaDespachoId === null,
        )
        .reduce((acc: number, item: any) => acc + (item.cantidad || 1), 0);
    ...
    const tabla = Array.from(mapaGrupos.values()).map((g) => {
      const procesadasTotal =
        g.preparadasSinDespacharEscuela + g.despachadasHistoricasEscuela;
      const avanceGlobal =
        g.totalPrendasEscuela > 0
          ? Math.round((procesadasTotal / g.totalPrendasEscuela) * 100)
          : 0;
...
export async function PUT(request: Request) {
  try {
    ...
    if (modo === "generar_guia") {
      ...
      for (const ped of pedidosInvolucrados) {
        const p = await prisma.pedido.findUnique({
          where: { id: ped.pedidoId },
          include: { detalles: true },
        });
        const tot = p?.detalles.length || 0;
        const desp =
          p?.detalles.filter((d) => d.guiaDespachoId !== null).length || 0;
        if (tot === desp && tot > 0) {
          await prisma.pedido.update({
            where: { id: ped.pedidoId },
            data: { estado: "Despachado", fechaFinEmpaque: new Date() },
          });
        }
      }
    ...
    const completados =
      pedidoActualizado?.detalles.filter(
        (d) => d.estadoEmpaque === "Preparado" || d.guiaDespachoId !== null,
      ).length || 0;

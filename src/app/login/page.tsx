...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      switch(errorParam) {
        case 'concurrencia':
          setError('Tu inicio de sesión fue registrado en otro dispositivo, alguien más inició sesión.');
          break;
        case 'inactividad':
          setError('Tu sesión se cerró por inactividad prolongada (30 min). Por favor, inicia sesión de nuevo.');
          break;
        case 'ip_invalida':
          setError('Acceso bloqueado. Estás intentando ingresar desde una red de internet no autorizada.');
          break;
        case 'bloqueado':
          setError('Tu cuenta ha sido bloqueada desde el sistema central. Contacta al Administrador.');
          break;
        default:
          setError('Tu sesión ha expirado por seguridad. Vuelve a iniciar sesión.');
      }
...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8 `bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))]` from-gray-50 to-gray-100">
      ...
      <Suspense fallback={<div className="mt-8 text-center text-gray-400 font-bold animate-pulse">Cargando módulo de seguridad...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  );

...
export function DashboardComercial() {
  ...
  useEffect(() => {
    const fetchCatalogos = async () => {
      try {
        const [resCat, resVend] = await Promise.all([
          fetch("/api/catalogos"),
  ...
  const cantonesDisponibles =
    filtros.provinciaId && catalogos?.provincias
      ? catalogos.provincias.find(
          (p: any) => p.id === parseInt(filtros.provinciaId),
        )?.cantones || []
      : [];
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col xl:flex-row gap-3 items-end xl:items-center justify-between">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 w-full">
          ...
          <div>
            ...
            <select
              className="w-full h-9 border rounded-md px-2 text-xs bg-white mt-1 outline-none font-medium"
              value={filtros.tipoVentaId}
              onChange={(e) =>
                setFiltros({ ...filtros, tipoVentaId: e.target.value })
              }
            >
              <option value="">Todas</option>
        ...
        <button
          ...
          className="flex items-center gap-2 h-9 px-4 rounded-md bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold shrink-0 transition-colors"
        >
      ...
      {loading ? (
        <div className="h-64 flex flex-col items-center justify-center text-gray-400 gap-3">
          ...
          <p className="font-bold text-sm tracking-widest uppercase">
            Calculando indicadores en tiempo real...
          </p>
        </div>
      ) : !data ? (
        <div className="h-64 flex items-center justify-center text-red-500 font-bold">
          Error de conexión con el motor de base de datos.
        </div>
      ) : (
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-wide">
                  Ventas Registradas
                </h3>
              </div>
              <p className="text-xl md:text-2xl font-black text-gray-900">
                {formatearDinero(data.financiero?.ventasRegistradas)}
              </p>
            ...
            >
              ...
              <p className="text-xl md:text-2xl font-black text-gray-900">
                {formatearDinero(data.financiero?.ventasValidadas)}
              </p>
              <p className="text-[9px] text-gray-400 mt-1 font-medium">
                Aprobadas por Facturación
              </p>
            </div>
            ...
            >
              ...
              <p className="text-xl md:text-2xl font-black text-amber-600">
                {formatearDinero(data.financiero?.pendienteValidacion)}
              </p>
              <p className="text-[9px] text-gray-400 mt-1 font-medium">
                En revisión o con novedades
              </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-purple-200 shadow-sm border-t-4">
              ...
              <p className="text-xl md:text-2xl font-black text-purple-700">
                {formatearDinero(data.financiero?.metaComercial)}
              </p>
              <p className="text-[9px] text-gray-400 mt-1 font-medium">
                Meta asignada al período
              </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-emerald-200 shadow-sm border-t-4">
              ...
              <p className="text-xl md:text-2xl font-black text-emerald-600">
                {data.financiero?.cumplimientoValidado}%
              </p>
              <p className="text-[9px] text-gray-400 mt-1 font-medium">
                Ventas validadas / Meta
              </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-blue-200 shadow-sm border-t-4">
              ...
              <p className="text-xl md:text-2xl font-black text-blue-600">
                {data.financiero?.cumplimientoPotencial}%
              </p>
              <p className="text-[9px] text-gray-400 mt-1 font-medium">
                Ventas registradas / Meta
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between">
              ...
              <div className="flex gap-4 items-center justify-center flex-1">
                <div
                  className="relative w-28 h-28 rounded-full shadow-inner flex items-center justify-center"
                  style={{ background: stringDona }}
                >
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-lg font-black text-red-600 underline decoration-red-200">
                      {data.campo?.visitasVencidas}
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs font-black text-gray-900">
                    {data.campo?.coberturaTerritorial}%
                  </span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2.5">
                  <div
                    className="bg-emerald-500 h-2.5 rounded-full"
                    style={{ width: `${data.campo?.coberturaTerritorial}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
              <h3 className="text-xs font-black text-gray-800 uppercase mb-4 text-center">
                Embudo Comercial
              </h3>
              <div
              ...
              >
                ...
                <div className="bg-blue-500 text-white text-[11px] font-bold py-1.5 w-[90%] text-center rounded-sm shadow-sm flex justify-between px-3">
                  <span>{data.campo?.instVisitadas}</span>{" "}
                  <span>Visitadas</span>
                </div>
                <div className="bg-purple-500 text-white text-[11px] font-bold py-1.5 w-[80%] text-center rounded-sm shadow-sm flex justify-between px-3">
                  <span>{data.campo?.visitasRealizadas}</span>{" "}
                  <span>Reuniones Ef.</span>
                </div>
                <div className="bg-amber-500 text-white text-[11px] font-bold py-1.5 w-[70%] text-center rounded-sm shadow-sm flex justify-between px-3">
                  <span>{data.campo?.cierres}</span> <span>Cierres</span>
                </div>
                <div className="bg-red-500 text-white text-[11px] font-bold py-1.5 w-[60%] text-center rounded-sm shadow-sm flex justify-between px-3">
                  <span>
                    {data.financiero?.ventasValidadas > 0 ? "Sí" : "0"}
                  </span>{" "}
                  <span>Validadas</span>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                      ...
                      data.ranking?.map((v: any) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          <td className="p-3 font-bold text-gray-800 flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[9px]">
                              {v.nombre.charAt(0)}
                            </div>{" "}
                            {v.nombre}
                          </td>
                          <td className="p-3 text-right font-black text-emerald-600">
                            {formatearDinero(v.ventaValidada)}
                          </td>
                          <td className="p-3 text-center">
                            <Badge
                              variant="outline"
                              className={
                                v.porcCumplimiento >= 100
                                  ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                  : v.porcCumplimiento >= 80
            ...
            <div className="lg:col-span-3 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <h3 className="text-xs font-black text-gray-800 uppercase mb-4 border-b border-gray-100 pb-2">
                Formalización de Ventas
              </h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-600 font-medium">
                    Contratos pendientes envío
                  </span>
                  <span className="font-bold">
                    {data.formalizacion?.contratosPendientesEnvio}
                  </span>
                </div>
                <div
                  className="flex justify-between items-center text-xs cursor-pointer hover:text-amber-600"
                  onClick={() => router.push("/ventas?tarjeta=Pendientes")}
                >
                  <span className="text-gray-600 font-medium">
                  ...
                  <span className="font-bold">
                    {data.formalizacion?.pendientesAuditoria}
                  </span>
                </div>
                <div
                ...
                >
                  ...
                  <span className="font-bold text-red-600">
                    {data.formalizacion?.contratosConNovedades}
                  </span>
                </div>
                <div
                  className="flex justify-between items-center text-xs cursor-pointer hover:text-orange-600"
                  onClick={() => router.push("/ventas")}
                >
                  <span className="text-gray-600 font-medium">
                    Tickets abiertos
                  ...
                  <span className="font-bold">
                    {data.formalizacion?.ticketsAbiertos}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-bold text-red-600">
                    {data.formalizacion?.ticketsVencidos}
                  </span>
                </div>
                <div className="mt-4 pt-4 border-t border-gray-100 bg-gray-50 p-2 rounded-lg text-center">
                  ...
                  <p className="text-xl font-black text-emerald-600">
                    {data.formalizacion?.indiceFormalizacion}%
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            <h3 className="text-xs font-black text-gray-800 uppercase mb-3">
              Atención Requerida (Alertas)
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              <div
                onClick={() => router.push("/agenda?tab=vencidas")}
                className="bg-red-50 p-3 rounded-xl border border-red-100 flex items-center gap-3 hover:bg-red-100 hover:-translate-y-1 shadow-sm transition-all cursor-pointer"
              >
              ...
              <div
                onClick={() => router.push("/ventas?tarjeta=Novedades")}
                className="bg-amber-50 p-3 rounded-xl border border-amber-100 flex items-center gap-3 hover:bg-amber-100 hover:-translate-y-1 shadow-sm transition-all cursor-pointer"
              >
              ...
              <div
                onClick={() => router.push("/pedidos")}
                className="bg-purple-50 p-3 rounded-xl border border-purple-100 flex items-center gap-3 hover:bg-purple-100 hover:-translate-y-1 shadow-sm transition-all cursor-pointer"
              >
                <PackageOpen size={24} className="text-purple-500 shrink-0" />
                <div>
                  <p className="text-[10px] font-bold text-purple-800 uppercase leading-tight">
                    Pedidos Borrador
                  </p>
              ...
              <div
                onClick={() => router.push("/pedidos")}
                className="bg-rose-50 p-3 rounded-xl border border-rose-100 flex items-center gap-3 hover:bg-rose-100 hover:-translate-y-1 shadow-sm transition-all cursor-pointer"
              >
              ...
              <div
                onClick={() => router.push("/ventas")}
                className="bg-orange-50 p-3 rounded-xl border border-orange-100 flex items-center gap-3 hover:bg-orange-100 hover:-translate-y-1 shadow-sm transition-all cursor-pointer"
              >
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center gap-3 hover:bg-slate-100 transition-colors">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-800 uppercase leading-tight">
                    Vend. &lt; 50% Meta
                  </p>
                  <p className="text-lg font-black text-slate-600">
                    {
                      data.ranking?.filter((v: any) => v.porcCumplimiento < 50)
                        .length
                    }
                  </p>
                </div>

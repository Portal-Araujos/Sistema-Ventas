  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (filtros.provinciaId && catData.provincias) {
        const prov = catData.provincias.find(
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <select
            className="h-9 border rounded-md px-2 text-xs bg-white w-32"
        ...
        {stats?.userRol !== "vendedor" && (
          <div>
            ...
            <select
              className="h-9 border rounded-md px-2 text-xs bg-white w-36"
        ...
        <Button
          className="h-9 text-xs font-bold bg-slate-900 hover:bg-black text-white"
          onClick={cargarDatos}
        >
      ...
      {loading ? (
        <div className="w-full h-64 flex flex-col items-center justify-center bg-white rounded-xl border border-gray-200">
          ...
          <p className="mt-4 text-sm text-gray-500 font-bold">
            Consolidando Centro de Mando...
          </p>
        </div>
      ) : (
        <>
          {/* 💰 FILA 1: KPIs FINANCIEROS Y COMERCIALES */}
          <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mt-2">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-3xl font-black mt-1">
                    {formatMoneda(stats?.kpis?.totalMontoVentas || 0)}
                  </h3>
                </div>
          ...
          <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mt-4">
            2. Rendimiento Logístico y Fábrica
          </h2>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-black text-gray-800">
                  {stats?.kpis?.despachadas || 0}
                </h3>
              </div>
            </div>

            <div
              className={`p-5 rounded-2xl border shadow-sm flex items-center gap-4 ${stats?.kpis?.prendasAtrasadas > 0 ? "bg-red-50 border-red-200" : "bg-white border-gray-200"}`}
            >
              <div
                className={`${stats?.kpis?.prendasAtrasadas > 0 ? "bg-red-200 text-red-700" : "bg-gray-100 text-gray-400"} p-3 rounded-full`}
              >
                <AlertTriangle size={20} />
              </div>
              <div>
                <p
                  className={`text-[10px] font-bold uppercase ${stats?.kpis?.prendasAtrasadas > 0 ? "text-red-500" : "text-gray-400"}`}
                >
                  Prendas Atrasadas
                </p>
                <h3
                  className={`text-2xl font-black ${stats?.kpis?.prendasAtrasadas > 0 ? "text-red-700" : "text-gray-800"}`}
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
              <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2 mb-6">
                <TrendingUp size={16} className="text-indigo-600" /> Rendimiento
                por Vendedor (Top 5)
              </h2>
              <div className="h-64 w-full">
                {stats?.graficos?.rendimientoVendedores?.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart
                      data={stats.graficos.rendimientoVendedores}
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
              <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2 mb-6">
                <Package size={16} className="text-blue-600" /> Distribución de
                Producción (Embudo)
              </h2>
              <div className="h-64 w-full">
                {stats?.graficos?.embudoProduccion?.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={stats.graficos.embudoProduccion}
                        cx="50%"
                        cy="50%"
                        innerRadius={60}
                        outerRadius={90}
                        ...
                        dataKey="value"
                      >
                        {stats.graficos.embudoProduccion.map(
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-2">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-gray-50/50">
                <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <DollarSign size={16} className="text-emerald-600" /> Últimos
                  Contratos Cerrados
                </h2>
              </div>
              <Table>
                ...
                <TableBody>
                  {stats?.tablas?.ultimasVentas?.length > 0 ? (
                    stats.tablas.ultimasVentas.map((v: any) => (
                      <TableRow key={v.id} className="text-xs font-semibold">
                        <TableCell className="text-gray-500">
                          {v.fecha}
                        </TableCell>
                        <TableCell className="text-gray-900 truncate max-w-150px">
                          {v.escuela}
                        </TableCell>
                        {stats?.userRol !== "vendedor" && (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-gray-100 bg-red-50/50">
                <h2 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <Clock size={16} className="text-red-600" /> Entregas Urgentes
                  (Fábrica)
                </h2>
              </div>
              <Table>
                ...
                <TableBody>
                  {stats?.tablas?.entregasUrgentes?.length > 0 ? (
                    stats.tablas.entregasUrgentes.map((u: any) => (
                      <TableRow key={u.id} className="text-xs font-semibold">
                        <TableCell className="text-red-600 font-black">
                          {u.fecha}
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Badge
                            variant="outline"
                            className="text-[9px] bg-amber-50"
                          >
                            {u.estado}
                          </Badge>
                        </TableCell>

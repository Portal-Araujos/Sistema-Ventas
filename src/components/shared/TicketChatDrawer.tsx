...
export default function TicketChatDrawer({ isOpen, onClose, ticketId, currentUserId, currentUserRol }: TicketChatDrawerProps) {
  ...
  const borrarArchivo = async (mensajeId: number) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/tickets?mensajeId=${mensajeId}`, { method: 'DELETE' });
      if (res.ok) {
        await cargarTicket();
      } else {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {!ticket ? (
            ...
            ticket.mensajes?.map((msg: any, idx: number) => {
              ...
              const isImage = msg.adjuntoTipo === 'imagen' || msg.adjuntoTipo?.includes('image');
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 shadow-sm text-sm ${isMe ? 'bg-primary text-white rounded-tr-none' : 'bg-white border border-gray-200 text-gray-800 rounded-tl-none'}`}>
                        ...
                        {isImage ? (
                          <div className="relative inline-block">
                            ...
                            <div className="absolute -top-3 -right-3 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              {(isMe || currentUserRol === 'super_admin' || msg.esSistema) && (
                                <button onClick={() => borrarArchivo(msg.id)} className="bg-red-500 text-white p-1.5 rounded-full shadow-md hover:bg-red-600 transition-colors" title="Borrar del Servidor">
                                  <Trash2 size={14}/>
                                </button>
                              )}
                            </div>
                          ...
                          <div className="relative inline-block w-full">
                            <a href={msg.adjuntoUrl} target="_blank" rel="noreferrer" className={`flex items-center gap-2 p-2 rounded-lg text-xs font-bold ${isMe ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                              <FileText size={16} /> {msg.adjuntoNombre || 'Documento Adjunto'}
                            </a>
                            
                            <div className="absolute -top-3 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <a href={msg.adjuntoUrl} download={msg.adjuntoNombre || 'documento.pdf'} className="bg-blue-500 text-white p-1.5 rounded-full shadow-md hover:bg-blue-600 transition-colors" title="Descargar documento">
                                <Download size={12}/>
                              </a>
                              {(isMe || currentUserRol === 'super_admin' || msg.esSistema) && (
                                <button onClick={() => borrarArchivo(msg.id)} className="bg-red-500 text-white p-1.5 rounded-full shadow-md hover:bg-red-600 transition-colors" title="Borrar del Servidor">
                    ...
                    {!msg.adjuntoUrl && msg.adjuntoNombre?.includes('eliminado') && (
                      <div className="mt-2 border-t border-white/20 pt-2 flex items-center gap-1">
                        ...
                        <p className={`text-[10px] italic font-medium ${isMe ? 'text-white/70' : 'text-gray-400'}`}>
                          {msg.adjuntoNombre}
                        </p>
                      </div>
                    )}
        ...
        {archivoAdjunto && (
          <div className="bg-blue-50 border-t border-blue-200 p-2 px-4 flex justify-between items-center">
            <div className="flex items-center gap-2 text-xs font-bold text-blue-800">
              {archivoAdjunto.type.startsWith('image/') ? <ImageIcon size={14}/> : <FileText size={14}/>}
              <span className="truncate max-w-200px">{archivoAdjunto.name}</span>
              <span className="text-[10px] text-blue-500">({(archivoAdjunto.size / 1024).toFixed(1)} KB)</span>
            </div>
        ...
        <div className="bg-white border-t border-gray-200 p-3 z-10">
          ...
          {ticket?.estado !== 'Cerrado' ? (
            <form onSubmit={enviarMensaje} className="flex gap-2 items-end">
              ...
              <Button type="submit" disabled={isCompressing || loading || (!mensaje.trim() && !archivoAdjunto)} className="bg-primary hover:bg-primary/90 text-white rounded-full p-2 h-10 w-10 shrink-0 flex items-center justify-center shadow-md">
